...
# ₹999/month | ₹8,999/year — Both Founding Member, locked forever
...
[data-baseweb="popover"] ul li { color: #FFD700 !important; }
[data-baseweb="popover"] ul li:hover { background: #1A3A6E !important; color: #FFFFFF !important; }
[data-baseweb="option"] { color: #FFD700 !important; background: #0D2157 !important; }
[data-baseweb="option"]:hover { background: #1A3A6E !important; color: #FFFFFF !important; }
/* Selected value in selectbox */
[data-baseweb="select"] [data-testid="stMarkdownContainer"] p { color: #FFD700 !important; }
[data-baseweb="select"] div[class*="ValueContainer"] { color: #FFD700 !important; }
[data-baseweb="select"] span { color: #FFD700 !important; }
/* Radio button labels */
.stRadio label { color: #FFD700 !important; }
.stRadio div[role="radiogroup"] label p { color: #FFD700 !important; }
.stRadio [data-testid="stMarkdownContainer"] p { color: #FFD700 !important; }
...
[data-testid="stFileUploader"]{background:#0D2157!important;border:1.5px dashed #1A3A6E!important;border-radius:8px!important;}
[data-testid="stFileUploader"]:hover{border-color:#00CFFF!important;}
[data-testid="stFileUploader"] section{background:transparent!important;}
[data-testid="stFileUploader"] section>button{background:#071233!important;color:#00CFFF!important;border:1px solid #1A3A6E!important;border-radius:6px!important;}
[data-testid="stFileUploader"] section p,[data-testid="stFileUploader"] section span,
[data-testid="stFileUploaderDropzoneInstructions"] span,[data-testid="stFileUploaderDropzoneInstructions"] small,
[data-testid="stFileUploader"] label,[data-testid="stFileUploader"]>div>label,
[data-testid="stWidgetLabel"] span,.stFileUploader label{color:#FFFFFF!important;opacity:.85!important;}
[data-testid="stDownloadButton"]>button{background:linear-gradient(135deg,#0095BF,#00CFFF)!important;color:#0A1B44!important;border:none!important;border-radius:6px!important;font-weight:700!important;box-shadow:0 2px 12px rgba(0,207,255,.35)!important;}
[data-testid="stDownloadButton"]>button:hover{box-shadow:0 6px 24px rgba(0,207,255,.55)!important;transform:translateY(-1px)!important;}
[data-testid="stExpander"] summary p,[data-testid="stExpander"] summary span,
.streamlit-expanderHeader,.streamlit-expanderHeader p{color:#FFFFFF!important;}

/* ── Feedback expander — labels & text in yellow (matches calculators) ── */
[data-testid="stExpander"] [data-testid="stWidgetLabel"] span,
[data-testid="stExpander"] [data-testid="stWidgetLabel"] p,
[data-testid="stExpander"] label { color: #FFD700 !important; }
...
/* ══════════════════════════════════════════════════════════════
   ...
   Fixes: stat-grid crush, wealth/price/sovereign number overflow,
   eb-banner horizontal scroll, block-container tight padding.
   ══════════════════════════════════════════════════════════════ */
@media (max-width: 640px) {

    /* Main container — give breathing room on narrow screens */
    .main .block-container {
        padding: 1rem 0.75rem !important;
    }

    /* Stat grid — 4 columns is unreadable on phones, use 2 */
    .stat-grid {
    ...
    /* Wealth number — 2.8rem overflows on 360px screens */
    .wealth-number {
    ...
    /* Pricing amount — scale down so it fits inside the card */
    .price-amount {
        font-size: 1.8rem !important;
    }
    .price-card {
    ...
    /* Early bird banner — already has flex-wrap, reduce padding */
    .eb-banner {
        padding: 12px 14px !important;
        gap: 10px !important;
    }

    /* Tabs — reduce padding so all tabs fit without horizontal scroll */
    .stTabs [data-baseweb="tab"] {
        padding: 6px 10px !important;
        font-size: 0.82rem !important;
    }

    /* Cards — reduce inner padding on small screens */
    .card {
        padding: 14px !important;
    }
...
def check_special_login(email, password):
    """
    Staff-only special login. Sovereign now authenticates via DB like all users.
    """
...
def _hash_password(plain: str) -> str:
    """Hash a plaintext password. Uses bcrypt if available, else sha256 fallback."""
    if BCRYPT_AVAILABLE:
...
def _verify_password(plain: str, stored: str) -> bool:
    ...
    if stored.startswith("sha256$"):
        try:
            ...
            return False
    # Legacy plaintext — accept once, then caller should re-hash and save
    return stored == plain

def get_or_create_free_user(email, password):
    """Register or fetch a free user. Passwords are stored as bcrypt hashes."""
    try:
        existing = db("free_users").select("*")\
            .eq("email", email).maybe_single().execute()

        if existing.data:
            stored = existing.data.get("password_hash", "")
            if _verify_password(password, stored):
                # Upgrade legacy plaintext passwords to bcrypt on successful login
                if not stored.startswith("$2") and not stored.startswith("sha256$"):
                    db("free_users").update({"password_hash": _hash_password(password)})\
...
def save_testimonial(email, name, text, itc_found):
    """Save a CA testimonial — goes to Supabase for admin approval."""
    try:
        db("testimonials").insert({
            ...
            "text"      : text.strip(),
            "itc_found" : itc_found,
            "approved"  : False,          # admin sets True in Supabase dashboard
            "created_at": now_iso()
        }).execute()
        return True
    except Exception as e:
...
def get_total_itc_wealth():
    """Sum of wealth_found across ALL recon_results rows."""
    try:
        res = db("recon_results").select("wealth_found").execute()
        return sum(r.get("wealth_found", 0) for r in (res.data or []))
    except Exception:
        return 0
...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    A. Unclaimed ITC Opportunity = max(2B - 3B, 0)
    ...
    IGST = ["itc_igst","igst","integrated_tax","integrated tax","igst_amount",
            ...
            # Generic single-column ITC names common in 2B/ITC Register exports
            "itc amount","itc_amount","itc","input tax credit",
            "total itc","total_itc","itc availed","itc_availed",
            "eligible itc","eligible_itc","net itc","net_itc"]
    ...
    IGST_3B = ["itc_claimed_igst","itc_availed_igst","itc_igst","igst",
               ...
               "igst_itc_claimed",
               "total itc claimed","total_itc_claimed","itc claimed","itc_claimed",
               "total itc","total_itc","itc amount","itc_amount",
               "itc","input tax credit","tax credit claimed"]
    ...
    # ── Auto-detect whether to include GSTIN in match key ──────────
    # Uses groupby (not set_index) to correctly handle duplicate invoice numbers
    # across different suppliers in large datasets.
    def _gstin_match_rate(sample=300):
        if gstin_col_2b is None or gstin_col_1 is None:
            return 0.0
        common_invs = list(set(gstr2b_df["_inv"].tolist()) & set(gstr1_df["_inv"].tolist()))[:sample]
        if not common_invs:
            return 0.0
        g1_grp  = gstr1_df[gstr1_df["_inv"].isin(common_invs)].groupby("_inv")["_gstin"].first()
        g2b_grp = gstr2b_df[gstr2b_df["_inv"].isin(common_invs)].groupby("_inv")["_gstin"].first()
        shared  = g1_grp.index.intersection(g2b_grp.index)
        if len(shared) == 0:
            return 0.0
        hits = sum(
            1 for inv in shared
            if g1_grp[inv] == g2b_grp[inv]
            and g1_grp[inv] not in ("UNKNOWN", "NAN", "NONE", "")
        )
        return hits / len(shared)

    # ── Auto-detect whether dates are consistent across files ───────
    # If dates never align (different formats that pandas couldn't parse),
    # drop date from the key to avoid every invoice becoming a mismatch.
    def _date_match_rate(sample=300):
        common_invs = list(set(gstr2b_df["_inv"].tolist()) & set(gstr1_df["_inv"].tolist()))[:sample]
        if not common_invs:
            return 0.0
        g1_grp  = gstr1_df[gstr1_df["_inv"].isin(common_invs)].groupby("_inv")["_date"].first()
        g2b_grp = gstr2b_df[gstr2b_df["_inv"].isin(common_invs)].groupby("_inv")["_date"].first()
        shared  = g1_grp.index.intersection(g2b_grp.index)
        if len(shared) == 0:
            return 0.0
        hits = sum(
            1 for inv in shared
            if g1_grp[inv] == g2b_grp[inv]
            and g1_grp[inv] not in ("NODATE", "NAN", "NONE", "")
        )
        return hits / len(shared)

    _use_gstin = _gstin_match_rate() >= 0.5
    _use_date  = _date_match_rate()  >= 0.3   # lower threshold — even partial date agreement is useful

    if _use_gstin and _use_date:
        gstr2b_df["_ckey"] = gstr2b_df["_gstin"] + "||" + gstr2b_df["_inv"] + "||" + gstr2b_df["_date"]
        gstr1_df["_ckey"]  = gstr1_df["_gstin"]  + "||" + gstr1_df["_inv"]  + "||" + gstr1_df["_date"]
    elif _use_gstin and not _use_date:
        gstr2b_df["_ckey"] = gstr2b_df["_gstin"] + "||" + gstr2b_df["_inv"]
        gstr1_df["_ckey"]  = gstr1_df["_gstin"]  + "||" + gstr1_df["_inv"]
    elif not _use_gstin and _use_date:
        gstr2b_df["_ckey"] = gstr2b_df["_inv"] + "||" + gstr2b_df["_date"]
        gstr1_df["_ckey"]  = gstr1_df["_inv"]  + "||" + gstr1_df["_date"]
    else:
        # Fallback: InvNo only — most permissive, avoids 100% mismatch on bad date formats
        gstr2b_df["_ckey"] = gstr2b_df["_inv"]
        gstr1_df["_ckey"]  = gstr1_df["_inv"]
    ...
    # ── GSTIN collision fix (InvNo-only key mode) ──────────────────
    # When matching on InvNo only, different suppliers can share the same invoice
    # number — producing false "matched" rows with GSTINs that don't agree.
    # These are not value mismatches; each side should be treated as a separate
    # missing invoice (ITC side → Missing in GSTR-1, TAX side → Missing in 2B).
    #
    # SAFETY CHECK: If >60% of matched rows have GSTIN clashes, the GSTINs are
    # likely unreliable across these files (different systems, test data, etc.).
    # In that case, skip the split — match on InvNo only and ignore GSTINs.
    if not _use_gstin and "_gstin_2b" in merged.columns and "_gstin_1" in merged.columns:
        has_both = (merged["_itc"] > 0) & (merged["_tax"] > 0)
        gstin_clash = (
            merged["_gstin_2b"].notna() & merged["_gstin_1"].notna() &
            (merged["_gstin_2b"] != merged["_gstin_1"]) &
            (~merged["_gstin_2b"].isin(["UNKNOWN", "NAN", "NONE", ""])) &
            (~merged["_gstin_1"].isin(["UNKNOWN", "NAN", "NONE", ""]))
        )
        false_rows = has_both & gstin_clash
        matched_count = has_both.sum()
        clash_rate = false_rows.sum() / matched_count if matched_count > 0 else 0.0
        # Only split if clash rate is low — high clash rate means GSTINs are
        # not comparable across files, so trust InvNo match and ignore GSTIN diff
        if false_rows.any() and clash_rate <= 0.6:
            fm = merged[false_rows].copy()
            # Split into two separate rows: one "missing in 2B", one "missing in GSTR-1"
            fm_miss2b = fm.copy(); fm_miss2b["_itc"] = 0.0  # G1 side only → missing in 2B
            fm_miss1  = fm.copy(); fm_miss1["_tax"]  = 0.0  # 2B side only → missing in GSTR-1
            merged = pd.concat(
                [merged[~false_rows], fm_miss2b, fm_miss1], ignore_index=True
            )
            merged["_delta"] = merged["_itc"] - merged["_tax"]
    ...
    _concat_parts = [p for p in [disp_missing_2b, disp_missing_1, disp_value_mm] if not p.empty]
    if _concat_parts:
        all_mismatches = pd.concat(_concat_parts, ignore_index=True)
        if "Delta (₹)" in all_mismatches.columns:
            all_mismatches["Delta (₹)"] = all_mismatches["Delta (₹)"].fillna(0)
            all_mismatches = all_mismatches.sort_values("Delta (₹)", key=lambda x: x.abs(), ascending=False)
    else:
    ...
    # Use unique _ckey counts — after GSTIN-collision splitting, the same key can produce
    # two rows (miss2b + miss1). Count unique keys flagged to avoid >100% mismatch rate.
    _mm_keys = set()
    if "_ckey" in missing_in_2b.columns:  _mm_keys |= set(missing_in_2b["_ckey"].dropna())
    if "_ckey" in missing_in_1.columns:   _mm_keys |= set(missing_in_1["_ckey"].dropna())
    if "_ckey" in value_mismatch.columns: _mm_keys |= set(value_mismatch["_ckey"].dropna())
    mismatch_total  = len(_mm_keys)
    ...
    # If integrity score is near 0% despite high invoice overlap, the files
    # likely have inconsistent GSTINs/dates/amounts across systems.
    # This is common with test data or files exported from different software.
    inv_overlap_rate = len(set(gstr2b_df["_inv"]) & set(gstr1_df["_inv"])) / max(len(set(gstr2b_df["_inv"])), 1)
    data_quality_warning = (recon_integrity < 5.0 and inv_overlap_rate > 0.5)
...
def _send_email(to_email: str, subject: str, html_body: str) -> bool:
    """Send email using Gmail SMTP. Returns True if sent."""
    ...
    try:
        ...
        # show error once in logs instead of silently failing
...
def send_welcome_email_subscriber(to_email: str, plan: str,
                                   founding_member: bool, amount: int):
    """
    Sent when someone converts from free to paid.
    Confirms the Founding Member lock in writing — reduces cancellation anxiety,
    ...
    eb_block = f"""
        <div style="background:#071233;border:1px solid #00CFFF;
          ...
          <p style="color:#00CFFF;font-weight:700;font-size:15px;
                    margin:0 0 10px 0;">🔥 Your Founding Member price is confirmed and locked</p>
          <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0;">
            You subscribed during the Founding Member period.
            ...
            <strong>One thing to remember:</strong> The Founding Member rate stays
    ...
    body = f"""
        ...
        <div style="background:#0D2157;border:1px solid #1A3A6E;
          ...
          <p style="color:#FFFFFF;font-size:13px;line-height:1.8;margin:0;">
            ...
            Tell a CA colleague. They get the same Founding Member pricing you did —
    ...
    subject = (
        f"Your Audit Desk {'Founding Member ' if founding_member else ''}subscription is confirmed"
    )
...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    story.append(Paragraph("A   UNCLAIMED ITC OPPORTUNITY", S_bold))
    ...
    data_a = [
        ...
        ["UNCLAIMED ITC OPPORTUNITY", _pdf_rupee(opportunity)],
    ]
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        story.append(Paragraph(
            "<b>Key:</b>  "
            "Filing Rate = % of supplier invoices in your GSTR-2B.  |  "
            "Repeat Defaulter = filed under 50% of own invoices — consider ITC reversal.  |  "
            "Risk Score = ITC amount (60%) + filing failure rate (40%).",
            ParagraphStyle("SupNote", fontName=FONT,
                           textColor=colors.HexColor("#334155"),
                           fontSize=7, leading=12, wordWrap="LTR",
    ...
    if summary:
        ...
        def _ic(txt, bold=False):
            """Wrap a cell string in a Paragraph for proper word-wrap."""
            style = PS_cell_b if bold else PS_cell
            return Paragraph(str(txt), style)

        overclaim_interp = (
            "No excess ITC claim detected. ITC claimed is within available GSTR-2B limits."
            if overclaim_p == 0.0
            else f"Excess ITC of {overclaim_p}% claimed relative to GSTR-2B — reversal required."
        )
        intel_data = [
            [_ic("Metric", bold=True), _ic("Value", bold=True), _ic("Interpretation", bold=True)],
            [_ic("Recon Integrity Score"),
             _ic(f"{integrity}%"),
             _ic("Strong (>75%)  Moderate (50–75%)  Weak (<50%)")],
            [_ic("Invoice Mismatch Rate"),
             _ic(f"{mm_pct}%"),
             _ic(f"{summary.get('mismatch_count',0):,} of {summary.get('invoices_total', summary.get('invoices_2b',0)):,} combined unique invoices across GSTR-1 & GSTR-2B")],
            [_ic("Net GST Exposure"),
             _ic(_pdf_rupee(net_exp)),
             _ic("Overclaim Risk + Value Mismatch Risk combined")],
            [_ic("Overclaim as % of 2B"),
             _ic(f"{overclaim_p}%"),
             _ic(overclaim_interp)],
            [_ic("Supplier Risk Concentration"),
             _ic(f"{top5_pct}%"),
             _ic(f"Top 5 suppliers account for {top5_pct}% of total supplier gap")],
            [_ic("Repeat Defaulters"),
             _ic(str(rep_count)),
             _ic(f"{rep_count} supplier(s) filed <50% of their own invoices — assess eligibility under Section 16(2) and evaluate the need for ITC reversal.")],
            [_ic("Highest Single Supplier Risk"),
             _ic(_pdf_rupee(top1_amt)),
             _ic(f"GSTIN: {summary.get('top1_gstin','')} — largest single supplier exposure")],
        ]
...
def page_login():
    ...
    # ── Auto-login via ?email=&session= sent from itcrecon.in ──
    url_email = params.get("email", "").strip().lower()
    url_token = params.get("session", "").strip()
    url_role  = params.get("role", "").strip()

    if url_email and url_token:
        try:
            # Verify token matches what we stored in DB
            fu_res = db("free_users").select("*").eq("email", url_email).maybe_single().execute()
            if fu_res.data and fu_res.data.get("session_token", "") == url_token:
                sov_email = _get_secret("SOVEREIGN_EMAIL", "__nobody__").lower()
                if url_email == sov_email or url_role == "sovereign":
                    S.logged_in = True; S.email = url_email; S.role = "sovereign"
                    S.is_subscriber = True
                    S.subscription = {"plan":"sovereign","founding_member":True,"amount":0,"status":"active"}
                    S.page = "sovereign"
                    st.query_params.clear(); st.rerun(); return
                if url_role == "staff":
                    S.logged_in = True; S.email = url_email; S.role = "staff"
                    S.is_subscriber = True
                    S.subscription = {"plan":"staff","founding_member":False,"amount":0,"status":"active"}
                    S.page = "dashboard"
                    st.query_params.clear(); st.rerun(); return
                # Check subscription
                sub = db("subscriptions").select("*").eq("email", url_email)                    .eq("status","active").maybe_single().execute()
                if sub.data:
                    _set_session_subscriber(url_email, sub.data)
                else:
                    _set_session_free(fu_res.data)
                st.query_params.clear(); st.rerun(); return
            else:
                st.error("Invalid or expired login link. Please sign in at itcrecon.in.")
...
def _do_login(email, password):
    # ── Staff special login (checked first, no DB needed) ──
    ...
    if role == "staff":
        ...
        S.role          = "staff"
        S.is_subscriber = True
        S.subscription  = {"plan": "staff", "founding_member": False,
                           "amount": 0, "status": "active"}
        S.page          = "dashboard"
        st.rerun()
        return

    # ── All users (including sovereign) — must authenticate via DB ──
    if not DB_LIVE:
    ...
    try:
        # Always check free_users first for password verification
        fu = db("free_users").select("*").eq("email", email).maybe_single().execute()
        if not fu.data:
            st.error("Invalid email or password.")
            return

        stored = fu.data.get("password_hash", "")
        if not _verify_password(password, stored):
            st.error("Invalid email or password.")
            return

        # Password is correct — upgrade legacy hash if needed
        if not stored.startswith("$2") and not stored.startswith("sha256$"):
            db("free_users").update({"password_hash": _hash_password(password)})\
                .eq("email", email).execute()

        # Sovereign — grant full sovereign session
        if email == sov_email:
            S.logged_in     = True
            S.email         = email
            S.role          = "sovereign"
            S.is_subscriber = True
            S.subscription  = {"plan": "sovereign", "founding_member": True,
                               "amount": 0, "status": "active"}
            S.page          = "sovereign"
            st.rerun()
            return
                ...
                try:
                    ...
                    if not reset_row.data:
                        ...
                        if datetime.now() > expiry:
                            ...
                            db("free_users").update({"password_hash": _hash_password(new_pass)})\
...
def _set_session_subscriber(email, sub_row):
    ...
    try:
        fu = db("free_users").select("welcome_sent,name").eq("email", email).execute()
        if fu.data and not fu.data[0].get("welcome_sent", False):
            _send_welcome_email(email=email, name=fu.data[0].get("name", ""))
    except Exception:
        pass


def _send_welcome_email(email: str, name: str = "") -> None:
    """Fire once on first login. Sends welcome + plan choice email."""
    greeting = f"Hi {name.split()[0]}," if name and name.strip() else "Hello,"
    body_html = f"""
    <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 16px 0;">
        {greeting}
    </p>
    <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 16px 0;">
        Your free account is live. Log in and run your first GST reconciliation
        &mdash; it takes under 30 seconds.
    </p>
    <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 24px 0;">
        Before you choose a plan, here is what is worth knowing.
    </p>
    <div style="border-left:3px solid #00CFFF;padding:4px 0 4px 16px;margin:0 0 24px 0;">
        <p style="color:#00CFFF;font-size:13px;font-weight:700;
                  letter-spacing:1px;text-transform:uppercase;margin:0 0 10px 0;">
            Both plans lock your price forever &mdash; or until we are here.
        </p>
        <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0 0 12px 0;">
            We say &ldquo;until we are here&rdquo; honestly. We are a small independent
            product. If we don&rsquo;t have enough subscribers to sustain, or a competitor
            outpaces us, or something unforeseen takes us out &mdash; we may cease to exist.
        </p>
        <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0;">
            Every subscriber who joins us &mdash; at &#8377;999 or &#8377;8,999 &mdash;
            is the reason we stay. Your subscription is not just a payment.
            It is a vote that keeps Audit Desk&#8482; alive. As long as we are here,
            your founding price is locked for life &mdash; as long as your subscription
            stays active and uninterrupted.
        </p>
    </div>
    <div style="background:#0D2157;border:1px solid #1A3A6E;border-radius:10px;
                padding:20px 24px;margin:0 0 24px 0;">
        <p style="color:#00CFFF;font-size:13px;font-weight:700;
                  letter-spacing:1px;text-transform:uppercase;margin:0 0 12px 0;">
            For Annual Founding Members
        </p>
        <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0 0 10px 0;">
            The annual plan is
            <strong style="color:#00CFFF;">&#8377;750 a month</strong>,
            billed once at &#8377;8,999.
        </p>
        <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0 0 16px 0;">
            When this tool recovers even &#8377;10,000 in missed ITC for one client
            &mdash; and for most CA firms it will &mdash; your annual subscription
            has paid for itself in a single reconciliation run.
        </p>
        <p style="color:#00CFFF;font-size:13px;font-weight:700;margin:0 0 8px 0;">
            One more thing &mdash; and this is our thank you.
        </p>
        <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0;">
            Your &#8377;8,999 does more than buy a subscription. It helps us build more.
            We are building more tools for the CA practice &mdash; we are not announcing
            what yet. But when we do, every Annual Founding Member gets to pick one future
            product that is useful to you and claim full access as a Founding Member
            at no extra cost, for as long as your annual subscription stays active
            and uninterrupted. Your seat is already reserved.
        </p>
    </div>
    <div style="text-align:center;margin:28px 0 20px 0;">
        <a href="https://itcrecon.in/?plan=annual#pricing"
           style="background:linear-gradient(135deg,#0095BF,#00CFFF);
                  color:#0A1B44;text-decoration:none;padding:14px 28px;
                  border-radius:8px;font-weight:700;font-size:14px;
                  display:inline-block;margin:0 8px 12px 8px;">
            Annual Founding Member &mdash; &#8377;8,999/year &#8594;
        </a>
        <a href="https://itcrecon.in/?plan=monthly#pricing"
           style="background:transparent;border:1px solid #00CFFF;
                  color:#00CFFF;text-decoration:none;padding:13px 28px;
                  border-radius:8px;font-weight:600;font-size:14px;
                  display:inline-block;margin:0 8px 12px 8px;">
            Join Monthly &mdash; &#8377;999/month &#8594;
        </a>
    </div>
    <p style="color:#FFFFFF;font-size:13px;text-align:center;margin:0 0 8px 0;opacity:.7;">
        Your price is locked from the moment you subscribe. Both plans. No exceptions.
    </p>
    <p style="color:#FFFFFF;font-size:14px;text-align:center;margin:0;line-height:1.8;">
        We are building this for you. Thank you for giving us the chance.
    </p>
    """
    try:
        db("free_users").update({"welcome_sent": True}).eq("email", email).execute()
    except Exception:
        pass
    _send_email(
        to_email=email,
        subject="Welcome to Audit Desk™ — read this before you subscribe",
        html_body=_email_html_wrapper(
            headline="We are genuinely glad you are here.",
            body_html=body_html
        )
    )
...
def _set_session_free(fu_row):
    ...
    if not fu_row.get("welcome_sent", False):
        try:
            _send_welcome_email(
                email=fu_row["email"],
                name=fu_row.get("name", "")
            )
        except Exception:
            pass
...
def sidebar():
    with st.sidebar:
        ...
        if role == "sovereign":
            ...
            eb    = sub.get("founding_member", False)
            badge = "🔐 Founding Member" if eb else "✅ Subscribed"
            st.markdown(f"""
        ...
        with st.expander("💬 Feedback / Ideas / Issues"):
            st.markdown("""
            <p style='color:#FFD700 !important; font-size:0.82rem; margin:0 0 8px 0;'>
            Tell us what you need &mdash; bugs, ideas, feature requests.<br>
            <span style='color:#00CFFF;'>Founding Members shape what we build next.</span><br>
            ...
            if st.button("Send Feedback", key="send_suggestion", use_container_width=True):
                if suggestion.strip():
                    ...
                    img_note = ""
                    img_b64 = None  # INITIALIZED TO PREVENT NAMEERROR
                    if fb_image:
                        img_b64 = _b64.b64encode(fb_image.read()).decode()
                        img_note = f" [screenshot: {fb_image.name}]"
                    full_msg = suggestion.strip() + img_note
                    try:
                        ...
                        pass
                    _send_email(
                        to_email="auditdesk.hq@gmail.com",
                        subject=f"Feedback from {S.email or 'anonymous'} — Audit Desk",
                        html_body=_email_html_wrapper(
                            headline="New Feedback / Idea",
                            body_html=(
                                f"<p><b>From:</b> {S.email or 'anonymous'}</p>"
                                f"<p><b>Message:</b><br>{full_msg.replace(chr(10), '<br>')}</p>"
                            )
                        )
                    )
                    st.success("✅ Received! We read every message.")
                else:
...
def page_dashboard():
    ...
    if not has_full_access():
        ...
        <b style='color:#00CFFF;'>🏅 Founding Member — ₹999/month or ₹8,999/year</b><br>
    ...
    if S.recon_result:
        ...
        if opportunity > 0:
            ...
            <div class='wealth-card'>
                <p class='wealth-label'>💎 UNCLAIMED ITC OPPORTUNITY</p>
        ...
        # ── Data quality warning ──────────────────────────────────────
        if summary.get("data_quality_warning"):
            st.warning(
                "⚠️ **Invoice matching quality alert** — The ITC totals (2B, 3B) are correct, "
                "but invoice-level matching shows near-100% mismatch. This usually means the "
                "GSTR-1 and GSTR-2B files use inconsistent Supplier GSTINs or tax amounts "
                "(common when files come from different software or are synthetic test data). "
                "**The ITC opportunity figure is reliable. Invoice-level stats should be verified "
                "with real client files.**"
            )
        ...
        if not S.recon_unlocked:
            ...
            <div style='background:#0D2157; border:2px solid #00CFFF;
                ...
                🏅 Founding Member — ₹999/month or ₹8,999/year · Both prices locked forever
                </p>
            </div>
            """, unsafe_allow_html=True)
        ...
        # ── 🌟 Testimonial Nudge — optional, never blocks PDF ──────────
        if opportunity > 0:
            st.markdown("---")
            # Use registered name if available, fall back to email prefix
            ca_display = (
                S.get("name") or S.get("full_name") or
                S.email.split("@")[0].title()
            ) if hasattr(S, "get") else S.email.split("@")[0].title()
            default_text = (
                f"Found \u20b9{opportunity:,.0f} in unclaimed ITC for my client "
                f"in under 30 seconds using Audit Desk Triangle Recon. "
                f"This would have taken me hours manually."
            )
            with st.expander("\u2b50 Impressed? Share your experience \u2014 optional", expanded=False):
                st.caption(
                    "Your testimonial may appear on itcrecon.in to help other CAs "
                    "discover this tool. You choose whether to submit — skip anytime."
                )
                ...
                t_text = st.text_area(
                    "Your experience (edit freely)",
                    value=default_text,
                    height=100,
                    key="t_text"
                )
                if st.button("\u2705 Submit Testimonial", type="primary", key="btn_testimonial"):
                    if len(t_text.strip()) < 20:
                        st.warning("Please write at least a sentence.")
                    else:
                        ok = save_testimonial(S.email, t_name, t_text, opportunity)
                        if ok:
                            st.success(
                                "\U0001f64f Thank you! Your testimonial has been received. "
                                "It will appear on itcrecon.in after a quick review."
                            )
                        else:
                            st.error("Could not save. Please try again.")

        # ── PDF always visible for subscribers, right here ────────────
        if S.recon_unlocked:
            _pdf_early = generate_pdf(
            ...
            with tab4:
                ...
                if si_df is not None and not si_df.empty:
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown(
                        "<div style='font-size:0.75rem;color:#FFFFFF;opacity:.75;"
                        "line-height:2;padding:8px 0;display:block;width:100%;'>"
                        "&#9679; Ranked by ITC at risk (highest first).<br>"
                        "&#9679; Filing Rate = % of supplier invoices in your GSTR-2B.<br>"
                        "&#9679; Repeat Defaulter = filed under 50% of own invoices "
                        "&#8212; consider ITC reversal.<br>"
                        "&#9679; Risk Score = ITC amount (60%) + filing failure rate (40%)."
                        "</div>",
                        unsafe_allow_html=True
                    )
    ...
    with tab1:
        ...
        with col_in:
            amt = st.number_input("Amount (₹)", min_value=0.0,
                                  value=10000.0, step=100.0, key="gst_amt")

            # Quick rate buttons
            st.markdown("<p style='color:#FFD700; font-size:0.85rem; margin-bottom:4px;'>Quick GST Rate</p>",
                        unsafe_allow_html=True)
            qb1, qb2, qb3, qb4 = st.columns(4)
            if "gst_rate_val" not in st.session_state:
                st.session_state["gst_rate_val"] = 18
            if qb1.button("5%",  key="r5",  use_container_width=True): st.session_state["gst_rate_val"] = 5
            if qb2.button("12%", key="r12", use_container_width=True): st.session_state["gst_rate_val"] = 12
            if qb3.button("18%", key="r18", use_container_width=True): st.session_state["gst_rate_val"] = 18
            if qb4.button("28%", key="r28", use_container_width=True): st.session_state["gst_rate_val"] = 28
            rate = st.session_state["gst_rate_val"]
            st.markdown(f"<p style='color:#00CFFF; font-size:0.8rem;'>Selected rate: <b>{rate}%</b></p>",
                        unsafe_allow_html=True)

            mode = st.radio("Mode",
                ["Exclusive (Add GST)", "Inclusive (Remove GST)"],
                horizontal=True, key="gst_mode")

            tx_type = st.radio("Transaction Type",
                ["Intra-State (CGST + SGST)", "Inter-State (IGST)"],
                horizontal=True, key="gst_txtype")

            if st.button("🔄 Reset", key="gst_reset", use_container_width=True):
                for k in ["gst_rate_val", "gst_amt", "gst_mode", "gst_txtype"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        with col_out:
            is_excl = "Exclusive" in mode
            is_igst = "Inter" in tx_type

            if is_excl:
                taxable = amt
                gst_amt = taxable * rate / 100
                total   = taxable + gst_amt
                lbl_gst = f"GST @{rate}%"
            else:
                total   = amt
                taxable = total / (1 + rate / 100)
                gst_amt = total - taxable
                lbl_gst = f"GST Extracted @{rate}%"
            ...
            # Build tax rows separately to avoid nested f-string issues
            if is_igst:
                tax_rows = (
                    "<div style='grid-column:span 2;'>"
                    "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>IGST @" + str(rate) + "%</p>"
                    "<p style='color:#00CFFF; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹" + f"{igst:,.2f}" + "</p>"
                    "</div>"
                )
            else:
                tax_rows = (
                    "<div>"
                    "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>CGST @" + str(rate // 2) + "%</p>"
                    "<p style='color:#00CFFF; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹" + f"{cgst:,.2f}" + "</p>"
                    "</div>"
                    "<div>"
                    "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>SGST @" + str(rate // 2) + "%</p>"
                    "<p style='color:#00CFFF; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹" + f"{sgst:,.2f}" + "</p>"
                    "</div>"
                )
            ...
            result_html = (
                ...
                "<p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.7rem;"
                "letter-spacing:2px; margin:0 0 14px 0;'>BREAKDOWN</p>"
                "<div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;'>"
                "<div>"
                "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Taxable Value</p>"
                "<p style='color:#FFD700; font-size:1.2rem; font-weight:700; margin:2px 0 10px 0;'>₹" + f"{taxable:,.2f}" + "</p>"
                "</div>"
                "<div>"
                "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>" + lbl_gst + "</p>"
                "<p style='color:#FFD700; font-size:1.2rem; font-weight:700; margin:2px 0 10px 0;'>₹" + f"{gst_amt:,.2f}" + "</p>"
                "</div>"
                + tax_rows +
                "</div>"
                "<hr style='border-color:#1A3A6E; margin:10px 0;'>"
                "<p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Total Invoice Value</p>"
                "<p style='color:#00CFFF; font-size:1.5rem; font-weight:700; margin:4px 0 0 0;'>₹" + f"{total:,.2f}" + "</p>"
                "</div>"
            )
            ...
            with st.expander("📋 Copy result as text"):
                if is_igst:
                    tax_line = f"IGST          : ₹{igst:,.2f}"
                else:
                    tax_line = f"CGST          : ₹{cgst:,.2f}\nSGST          : ₹{sgst:,.2f}"
                result_text = f"GST Calculation — {lbl_gst}\nTaxable Value : ₹{taxable:,.2f}\n{tax_line}\nTotal Invoice : ₹{total:,.2f}"
                st.code(result_text, language=None)

            with st.expander("📐 Formula"):
                if is_excl:
                    st.markdown(f"<span style='color:#FFFFFF;'>GST = Taxable × {rate}/100<br>Total = Taxable + GST</span>",
                                unsafe_allow_html=True)
                else:
                    st.markdown(f"<span style='color:#FFFFFF;'>Taxable = Total ÷ (1 + {rate}/100)<br>GST = Total − Taxable</span>",
                                unsafe_allow_html=True)
    # ── TAB 2: MSME 45-DAY ───────────────────────────────────────────
    with tab2:
        ...
        c1, c2 = st.columns(2)
        with c1:
            inv_date   = st.date_input("Invoice Date", key="msme_inv")
            inv_amount = st.number_input("Invoice Amount (₹)", min_value=0.0,
                                         value=0.0, step=1000.0, key="msme_amt")
            vtype = st.selectbox("Vendor Type", [
                "Micro Enterprise — 45 days",
                "Small Enterprise — 45 days",
                "Medium Enterprise — 45 days",
                "Non-MSME — Rule not applicable",
            ], key="msme_vtype")

            if st.button("🔄 Reset", key="msme_reset", use_container_width=True):
                for k in ["msme_inv", "msme_amt", "msme_vtype"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        with c2:
            if "Non-MSME" in vtype:
                st.markdown("""
                <div style='background:#071233; border:1px solid #1A3A6E;
                            border-radius:10px; padding:20px; margin-top:8px;'>
                    <p style='color:#FFD700; font-size:1rem; font-weight:700; margin:0;'>
                    ℹ️ Not Applicable</p>
                    <p style='color:#FFFFFF; font-size:0.9rem; margin:10px 0 0 0;'>
                    Section 43B(h) applies only to Micro and Small enterprises.<br>
                    Medium enterprises are also covered under the MSMED Act payment rules.
                    </p>
                </div>
                """, unsafe_allow_html=True)
            else:
                ...
                if left < 0:
                    status_color = "#FF4B4B"
                    status_bg    = "rgba(255,75,75,0.1)"
                    status_border= "#FF4B4B"
                    status_icon  = "🔴"
                    status_text  = f"{abs(left)} DAYS OVERDUE"
                elif left <= 7:
                    status_color = "#FFA500"
                    status_bg    = "rgba(255,165,0,0.1)"
                    status_border= "#FFA500"
                    status_icon  = "🟠"
                    status_text  = f"DUE IN {left} DAYS — URGENT"
                else:
                    status_color = "#00C853"
                    status_bg    = "rgba(0,200,83,0.1)"
                    status_border= "#00C853"
                    status_icon  = "🟢"
                    status_text  = f"{left} DAYS REMAINING — ON TRACK"

                amt_display = f"₹{inv_amount:,.2f}" if inv_amount > 0 else "—"
                risk_line = ""
                if left < 0 and inv_amount > 0:
                    risk_line = f"<p style='color:#FF4B4B; font-size:0.85rem; margin:10px 0 0 0;'>⚠️ Section 43B(h): ₹{inv_amount:,.2f} may be disallowed as expense until payment is made.</p>"
                elif left < 0:
                    risk_line = "<p style='color:#FF4B4B; font-size:0.85rem; margin:10px 0 0 0;'>⚠️ Section 43B(h): Expense may be disallowed until payment is made.</p>"

                st.markdown(f"""
                <div style='background:{status_bg}; border:1px solid {status_border};
                            border-radius:10px; padding:20px; margin-top:8px;'>
                    <p style='color:{status_color}; font-family:IBM Plex Mono;
                              font-size:0.75rem; letter-spacing:2px; margin:0 0 10px 0;'>
                    {status_icon} {status_text}</p>
                    <div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;'>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Invoice Date</p>
                            <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0;'>{inv_date.strftime("%d %b %Y")}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Due Date (45 days)</p>
                            <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0;'>{due.strftime("%d %b %Y")}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Invoice Amount</p>
                            <p style='color:#FFD700; font-size:1rem; font-weight:700; margin:2px 0;'>{amt_display}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Days Overdue</p>
                            <p style='color:{status_color}; font-size:1rem; font-weight:700; margin:2px 0;'>{max(0, abs(left)) if left < 0 else 0}</p>
                        </div>
                    </div>
                    {risk_line}
                </div>
                ...
                with st.expander("📋 Copy result as text"):
                    copy_text = f"""MSME 45-Day Checker — Section 43B(h)
Vendor Type   : {vtype}
Invoice Date  : {inv_date.strftime("%d %b %Y")}
Due Date      : {due.strftime("%d %b %Y")}
Invoice Amount: {amt_display}
Status        : {status_text}"""
                    if left < 0:
                        copy_text += f"\nSection 43B(h) Risk: Expense disallowance risk on ₹{inv_amount:,.2f}"
                    st.code(copy_text, language=None)

    # ── TAB 3: SECTION 50 INTEREST ───────────────────────────────────
    with tab3:
        log_calculator_use("sec50_interest", S.session_id)
        st.subheader("Section 50 — GST Interest Calculator")

        c1, c2 = st.columns(2)
        with c1:
            tax_amt   = st.number_input("Tax Amount (₹)", min_value=0.0,
                                         value=50000.0, step=1000.0, key="s50_tax")
            int_type  = st.radio("Interest Type", [
                "Late Payment / Short Payment — 18% p.a.",
                "Wrong ITC Utilization — 24% p.a."
            ], key="s50_type")
            due_date  = st.date_input("Due Date", key="s50_due")
            paid_date = st.date_input("Payment Date", key="s50_paid")

            if st.button("🔄 Reset", key="s50_reset", use_container_width=True):
                for k in ["s50_tax", "s50_type", "s50_due", "s50_paid"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        with c2:
            rate_pct  = 24.0 if "24%" in int_type else 18.0
            days_late = max(0, (paid_date - due_date).days)
            per_day   = tax_amt * (rate_pct / 100) / 365
            total_int = per_day * days_late
            total_pay = tax_amt + total_int
            type_lbl  = "Wrong ITC Utilization" if "24%" in int_type else "Late Payment"
            ...
            if paid_date < due_date:
                ...
                <div style='background:rgba(0,200,83,0.1); border:1px solid #00C853;
                    ...
                    <p style='color:#00C853; font-weight:700; margin:0;'>
                    ✅ Payment made before due date — no interest applicable.</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div style='background:#071233; border:1px solid #00CFFF;
                            border-radius:12px; padding:20px 24px; margin-top:8px;'>
                    <p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.7rem;
                              letter-spacing:2px; margin:0 0 14px 0;'>INTEREST CALCULATION</p>
                    <div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;'>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Type</p>
                            <p style='color:#FFD700; font-size:0.95rem; font-weight:700; margin:2px 0 10px 0;'>{type_lbl}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Rate</p>
                            <p style='color:#FFD700; font-size:0.95rem; font-weight:700; margin:2px 0 10px 0;'>{rate_pct:.0f}% p.a.</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Delay</p>
                            <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0 10px 0;'>{days_late} days</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Per-Day Interest</p>
                            <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0 10px 0;'>₹{per_day:,.2f}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Total Interest</p>
                            <p style='color:#FF4B4B; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹{total_int:,.2f}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Total Payable</p>
                            <p style='color:#00CFFF; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹{total_pay:,.2f}</p>
                        </div>
                    </div>
                    <hr style='border-color:#1A3A6E; margin:10px 0;'>
                    <p style='color:#FFFFFF; font-size:0.75rem; margin:0; opacity:.8;'>
                    Interest calculated @{rate_pct:.0f}% p.a. under Section 50 of the CGST Act.
                    Formula: Tax × Rate/100 × Days/365</p>
                </div>
                """, unsafe_allow_html=True)

                with st.expander("📋 Copy result as text"):
                    copy_text = f"""Section 50 GST Interest — {type_lbl}
Tax Amount    : ₹{tax_amt:,.2f}
Rate          : {rate_pct:.0f}% p.a.
Due Date      : {due_date.strftime("%d %b %Y")}
Payment Date  : {paid_date.strftime("%d %b %Y")}
Days Delayed  : {days_late}
Per-Day Int.  : ₹{per_day:,.2f}
Total Interest: ₹{total_int:,.2f}
Total Payable : ₹{total_pay:,.2f}"""
                    st.code(copy_text, language=None)

                with st.expander("📐 Formula"):
                    st.markdown(
                        f"<span style='color:#FFFFFF;'>Interest = Tax × ({rate_pct}/100) × (Days/365)<br>"
                        f"= ₹{tax_amt:,.2f} × {rate_pct/100} × ({days_late}/365)<br>"
                        f"= ₹{total_int:,.2f}</span>",
                        unsafe_allow_html=True)
    ...
    with tab4:
        ...
        st.caption("Search by code or description — type to filter")

        hsn_data = [
            # Goods — HSN
            ("0101","Live horses, asses, mules and hinnies","Nil"),
            ("0201","Meat of bovine animals, fresh/chilled","Nil"),
            ("1001","Wheat and meslin","Nil"),
            ("1006","Rice","Nil/5%"),
            ("2701","Coal; briquettes, ovoids and similar solid fuels","5%"),
            ("2710","Petroleum oils and oils from bituminous minerals","5%/18%"),
            ("3004","Medicaments for therapeutic/prophylactic use","12%"),
            ("3401","Soap, organic surface-active products","18%"),
            ("3923","Plastic articles for packaging goods","18%"),
            ("4901","Printed books, brochures, leaflets","Nil"),
            ("6101","Men's overcoats, car-coats, capes, cloaks","5%/12%"),
            ("7208","Flat-rolled products of iron or non-alloy steel","18%"),
            ("8471","Automatic data processing machines (computers/laptops)","18%"),
            ("8517","Telephone sets including smartphones","18%"),
            ("8703","Motor cars and other motor vehicles","28%+cess"),
            ("9403","Other furniture and parts thereof","18%"),
            ("9503","Toys, tricycles, scooters, pedal cars","12%"),
            # Services — SAC
            ("996111","SAC: Postal and courier services","18%"),
            ("996211","SAC: Road transport services of goods","5%/12%"),
            ("996311","SAC: Hotel accommodation services","12%/18%"),
            ("996411","SAC: Restaurant and food serving services","5%/18%"),
            ("997111","SAC: Construction of residential buildings","18%"),
            ("997112","SAC: Construction of non-residential buildings","18%"),
            ("997211","SAC: Renting of immovable property (commercial)","18%"),
            ("997212","SAC: Renting of immovable property (residential)","Nil/18%"),
            ("997311","SAC: Financial and related services","18%"),
            ("997321","SAC: Insurance and related services","18%"),
            ("998211","SAC: Legal advisory and representation services","18%"),
            ("998212","SAC: Legal documentation and certification services","18%"),
            ("998231","SAC: Accounting, auditing and bookkeeping services","18%"),
            ("998311","SAC: Management consulting and advisory services","18%"),
            ("998312","SAC: Business consulting services including PR","18%"),
            ("998313","SAC: IT consulting and support services","18%"),
            ("998314","SAC: Information technology (IT) design and development","18%"),
            ("998315","SAC: Hosting and IT infrastructure management","18%"),
            ("998316","SAC: IT technical support services","18%"),
            ("998399","SAC: IT software development and related services","18%"),
            ("998411","SAC: General construction services (civil)","18%"),
            ("998511","SAC: Temporary staffing and labour supply","18%"),
            ("998596","SAC: Facilities support services (facility management)","18%"),
            ("999111","SAC: Primary and secondary education services","Exempt"),
            ("999211","SAC: Higher education services","Exempt/18%"),
            ("999311","SAC: Health services by clinical establishments","Exempt"),
            ("999411","SAC: Sewage and waste collection services","Nil"),
        ]
        df_hsn = pd.DataFrame(hsn_data, columns=["Code","Description","GST Rate"])
        search = st.text_input("Search by description or code",
                               placeholder="e.g. legal services, construction, IT, 998311...",
                               key="hsn_search")
        if search:
        ...
        if not df_hsn.empty and search:
            with st.expander("📋 Copy matching codes"):
                lines = "\n".join(
                    f"{row['Code']} — {row['Description']} — {row['GST Rate']}"
                    for _, row in df_hsn.iterrows()
                )
                st.code(lines, language=None)

        st.caption("Simplified reference list. Always verify on the official GST portal.")

    # ── TAB 5: GSTR-3B LATE FEE CALCULATOR ──────────────────────────
    with tab5:
        log_calculator_use("gstr3b_latefee", S.session_id)
        st.subheader("GSTR-3B Late Fee Calculator")
        st.caption("Under Section 47 of the CGST Act")

        c1, c2 = st.columns(2)
        with c1:
            ret_type = st.selectbox("Return Type", [
                "GSTR-3B — Nil Return",
                "GSTR-3B — With Tax Liability",
            ], key="lf_type")
            delay_days = st.number_input("Days of Delay", min_value=0,
                                          value=0, step=1, key="lf_days")

            if st.button("🔄 Reset", key="lf_reset", use_container_width=True):
                for k in ["lf_type", "lf_days"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        with c2:
            is_nil   = "Nil" in ret_type
            cgst_daily = 25.0 if is_nil else 50.0
            sgst_daily = 25.0 if is_nil else 50.0
            max_cgst   = 250.0 if is_nil else 5000.0
            max_sgst   = 250.0 if is_nil else 5000.0

            raw_cgst = cgst_daily * delay_days
            raw_sgst = sgst_daily * delay_days
            fin_cgst = min(raw_cgst, max_cgst)
            fin_sgst = min(raw_sgst, max_sgst)
            total_fee = fin_cgst + fin_sgst

            capped = raw_cgst > max_cgst or raw_sgst > max_sgst

            st.markdown(f"""
            <div style='background:#071233; border:1px solid #00CFFF;
                        border-radius:12px; padding:20px 24px; margin-top:8px;'>
                <p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.7rem;
                          letter-spacing:2px; margin:0 0 14px 0;'>LATE FEE BREAKDOWN</p>
                <div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;'>
                    <div>
                        <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Daily Late Fee (CGST)</p>
                        <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0 10px 0;'>₹{cgst_daily:.0f}/day</p>
                    </div>
                    <div>
                        <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Daily Late Fee (SGST)</p>
                        <p style='color:#FFFFFF; font-size:1rem; font-weight:700; margin:2px 0 10px 0;'>₹{sgst_daily:.0f}/day</p>
                    </div>
                    <div>
                        <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>CGST Late Fee</p>
                        <p style='color:#FFD700; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹{fin_cgst:,.2f} {"(capped)" if capped else ""}</p>
                    </div>
                    <div>
                        <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>SGST Late Fee</p>
                        <p style='color:#FFD700; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹{fin_sgst:,.2f} {"(capped)" if capped else ""}</p>
                    </div>
                </div>
                <hr style='border-color:#1A3A6E; margin:10px 0;'>
                <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Total Late Fee</p>
                <p style='color:#00CFFF; font-size:1.5rem; font-weight:700; margin:4px 0;'>₹{total_fee:,.2f}</p>
                <p style='color:#FFFFFF; font-size:0.72rem; opacity:.8; margin:4px 0 0 0;'>
                Maximum cap: ₹{max_cgst+max_sgst:,.0f} {"(Nil return)" if is_nil else "(With tax liability)"}</p>
            </div>
            """, unsafe_allow_html=True)

            with st.expander("📋 Copy result as text"):
                copy_text = f"""GSTR-3B Late Fee Calculation
Return Type   : {ret_type}
Days of Delay : {delay_days}
CGST Late Fee : ₹{fin_cgst:,.2f}
SGST Late Fee : ₹{fin_sgst:,.2f}
Total Late Fee: ₹{total_fee:,.2f}
Maximum Cap   : ₹{max_cgst+max_sgst:,.0f}"""
                st.code(copy_text, language=None)

            with st.expander("📐 How this is calculated"):
                st.markdown(f"""
                <span style='color:#FFFFFF;'>
                • Nil return: ₹25/day (CGST) + ₹25/day (SGST), capped at ₹500 total<br>
                • With tax liability: ₹50/day (CGST) + ₹50/day (SGST), capped at ₹10,000 total<br>
                • Late fee = Daily rate × delay days (subject to cap)<br>
                • Under Section 47 of CGST Act, 2017
                </span>
                """, unsafe_allow_html=True)

    # ── TAB 6: ITC REVERSAL CALCULATOR (Rule 42/43) ──────────────────
    with tab6:
        log_calculator_use("itc_reversal", S.session_id)
        st.subheader("ITC Reversal Calculator — Rule 42 / Rule 43")
        st.caption("For businesses with both taxable and exempt supplies")

        c1, c2 = st.columns(2)
        with c1:
            rule_type = st.radio("Rule", [
                "Rule 42 — Inputs & Input Services",
                "Rule 43 — Capital Goods",
            ], key="rev_rule")
            total_itc    = st.number_input("Total ITC Claimed (₹)", min_value=0.0,
                                            value=0.0, step=1000.0, key="rev_itc")
            exempt_turn  = st.number_input("Exempt Turnover (₹)", min_value=0.0,
                                            value=0.0, step=10000.0, key="rev_exempt")
            total_turn   = st.number_input("Total Turnover (₹)", min_value=0.0,
                                            value=0.0, step=10000.0, key="rev_total")

            if st.button("🔄 Reset", key="rev_reset", use_container_width=True):
                for k in ["rev_rule", "rev_itc", "rev_exempt", "rev_total"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()
        ...
        with c2:
            if total_turn <= 0:
                st.info("Enter Total Turnover to calculate.")
            else:
                exempt_ratio  = min(exempt_turn / total_turn, 1.0)
                itc_to_reverse = total_itc * exempt_ratio
                itc_eligible   = total_itc - itc_to_reverse
                pct_reversed   = exempt_ratio * 100

                st.markdown(f"""
                <div style='background:#071233; border:1px solid #00CFFF;
                            border-radius:12px; padding:20px 24px; margin-top:8px;'>
                    <p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.7rem;
                              letter-spacing:2px; margin:0 0 14px 0;'>ITC REVERSAL RESULT</p>
                    <div style='display:grid; grid-template-columns:1fr 1fr; gap:10px;'>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Exempt Ratio</p>
                            <p style='color:#FFD700; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>{pct_reversed:.2f}%</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>Total ITC</p>
                            <p style='color:#FFFFFF; font-size:1.1rem; font-weight:700; margin:2px 0 10px 0;'>₹{total_itc:,.2f}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>ITC to Reverse</p>
                            <p style='color:#FF4B4B; font-size:1.2rem; font-weight:700; margin:2px 0 10px 0;'>₹{itc_to_reverse:,.2f}</p>
                        </div>
                        <div>
                            <p style='color:#FFFFFF; font-size:0.75rem; margin:0;'>ITC Eligible to Retain</p>
                            <p style='color:#00C853; font-size:1.2rem; font-weight:700; margin:2px 0 10px 0;'>₹{itc_eligible:,.2f}</p>
                        </div>
                    </div>
                    <hr style='border-color:#1A3A6E; margin:10px 0;'>
                    <p style='color:#FFFFFF; font-size:0.72rem; opacity:.8; margin:0;'>
                    {rule_type.split("—")[0].strip()} · Proportionate reversal method</p>
                </div>
                """, unsafe_allow_html=True)

                with st.expander("📋 Copy result as text"):
                    copy_text = f"""ITC Reversal — {rule_type}
Total ITC     : ₹{total_itc:,.2f}
Exempt Turn.  : ₹{exempt_turn:,.2f}
Total Turn.   : ₹{total_turn:,.2f}
Exempt Ratio  : {pct_reversed:.2f}%
ITC to Reverse: ₹{itc_to_reverse:,.2f}
ITC to Retain : ₹{itc_eligible:,.2f}"""
                    st.code(copy_text, language=None)

                with st.expander("📐 Formula"):
                    st.markdown(f"""
                    <span style='color:#FFFFFF;'>
                    Exempt Ratio = Exempt Turnover ÷ Total Turnover<br>
                    = ₹{exempt_turn:,.2f} ÷ ₹{total_turn:,.2f} = {pct_reversed:.2f}%<br><br>
                    ITC to Reverse = Total ITC × Exempt Ratio<br>
                    = ₹{total_itc:,.2f} × {pct_reversed:.2f}% = ₹{itc_to_reverse:,.2f}
                    </span>
                    """, unsafe_allow_html=True)
...
def page_subscribe():
    ...
    if eb_open:
        ...
        <div class='eb-banner'>
            ...
            <div>
                ...
                Founding Member Pricing — Limited Time</b><br>
                <span style='color:#FFFFFF;'>
    ...
    if S.is_subscriber and S.subscription:
        ...
        eb_locked     = sub.get("founding_member", False)
        st.markdown(f"""
        ...
        {"· 🔥 <b style='color:#00CFFF;'>Founding Member locked — this price is yours forever</b>"
         if eb_locked else ""}
        </span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class='price-card'>
            ...
            {"🔥 <b>Founding Member Price</b> — this ₹999 is locked forever" if eb_open else ""}
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class='price-card featured'>
            ...
            ✅ Priority support<br>
            {"🔥 <b>Founding Member Price</b> — this ₹8,999 is locked forever" if eb_open else ""}
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    🔒 <b style='color:#00CFFF;'>Both monthly and annual Founding Member prices are locked
    ...
    <span style='color:#FFFFFF;'>No card needed · cancel anytime</span>
    </div>
    """, unsafe_allow_html=True)

def get_renewal_amount(email, plan):
    """
    RENEWAL PRICE LOGIC — The Founding Member Annual Fix.

    Rule 1: If subscriber has founding_member = true on their existing
    ...
    Rule 2: If they cancelled and are rejoining, founding_member = false
    ...
    Returns: (amount, founding_member_flag, is_renewal)
    """
    try:
        ...
        if history.data:
            ...
            was_founding_member = last_sub.get("founding_member", False)
...
def _initiate_payment(plan, requested_amount):
    ...
    try:
        if razor_client:
            ...
            Founding Member: <b>{'Yes 🔥' if eb_flag else 'No'}</b>
...
def activate_subscription(email, plan, amount, founding_member):
    """
    Called by Razorpay webhook on payment success.
    Also used in simulation mode.

    CRITICAL RULE: Never overwrite founding_member = true with false.
    If a subscriber is renewing, UPDATE the existing row.
    If new, INSERT a new row.
    This preserves the founding_member flag through all renewals.
    """
    try:
        ...
        if existing.data:
            ...
            # Never touch founding_member — it was set at first subscription
            # and is permanent as long as they stay subscribed
            db("subscriptions").update({
                "renewed_at" : now_iso(),
                "amount"     : amount,
                "plan"       : plan,
                # founding_member is intentionally NOT updated here
        ...
        try:
            send_welcome_email_subscriber(
                email, plan, founding_member, int(amount))
        except:
            pass
        msg = "✅ Subscription activated!"
        if founding_member:
            msg += " 🏅 Founding Member price locked forever."
        st.success(msg)
        st.rerun()
...
def page_sovereign():
    ...
    for col, val, label, color in [
        ...
        (u4, f"{eb_used:,} / {EARLY_BIRD_LIMIT:,}", "Founding Member Slots Used", "#00CFFF"),
        (u5, f"{eb_slots:,}",       "Founding Member Slots Remaining","#FFFFFF"),
    ]:
        col.markdown(f"""
        <div style='background:#071233; border:1px solid #1A3A6E;
    ...
    try:
        ...
        if all_subs.data:
            ...
            show_cols = [c for c in
                ["email","plan","amount","founding_member","status",
            ...
            if "founding_member" in df_display.columns:
                df_display["founding_member"] = df_display["founding_member"].apply(
            ...
            try:
                ...
                if existing.data and len(existing.data) > 0:
                    ...
                    db("subscriptions").insert({
                        ...
                        "founding_member": False, "expires_at": new_exp.isoformat(),
