...
def page_login():
    ...
    if url_email:
        ...
        # Sovereign
        if url_role == "sovereign" and email == _get_secret("SOVEREIGN_EMAIL","").lower():
        ...
        # Regular user
        try:
            sub = db("subscriptions").select("*").eq("email", email)\
                .eq("status","active").maybe_single().execute()
            if sub.data:
                _set_session_subscriber(email, sub.data)
                st.query_params.clear(); st.rerun(); return
            fu = db("free_users").select("*").eq("email", email)\
                .maybe_single().execute()
            if fu.data:
                _set_session_free(fu.data)
                st.query_params.clear(); st.rerun(); return
            else:
                # Email not found in DB — show helpful message
                st.error(f"Account not found for {email}. Please register at itcrecon.in")
