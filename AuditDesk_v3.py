...
def page_login():
    ...
    # Auto-login via ?email=&session= param passed from itcrecon.in
    ...
    if url_email:
        ...
        # REQUIRE a valid session token — reject if missing or invalid
        if not url_token or not _verify_session_token(email, url_token):
            st.error("Invalid or expired login link. Please sign in manually.")
            st.query_params.clear()
            return

        # Token is valid — now grant access based on role
...
def _do_login(email, password):
    ...
    try:
        sub = db("subscriptions").select("*")\
            ...
            .maybe_single().execute()
        if sub.data:
            # Subscriber found — MUST verify password before granting access
            ...
            if fu.data:
                ...
                # Subscriber exists but no free_users row — ask them to set a password
                st.error("No password set for this account. Please use 'Forgot Password' to set one.")
                return
    except Exception:
        pass
