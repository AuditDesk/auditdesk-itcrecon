# ============================================================
# AUDIT DESK v3.0 — COMPLETE SUBSCRIPTION EDITION
# ₹999/month | ₹8,999/year | First 5,000 = Early Bird Forever
# Stack: Streamlit + Supabase + Razorpay
# Deploy: Streamlit Cloud (free) — Zero developers needed
# ============================================================
...
.stButton > button[kind="primary"]:hover {
    ...
    background: linear-gradient(135deg, #00CFFF 0%, #C2EDFB 100%) !important;
}
...
[data-testid="stMetricLabel"]     { color: #C2EDFB !important; }
...
.stat-label { color: #93C8E0; font-size: 0.78rem; margin: 4px 0 0 0; }
...
.price-period { color: #93C8E0; font-size: 0.85rem; }
</style>
...
SITE_URL           = "https://auditdesk.streamlit.app"  # ← change after deploy
...
def init_session():
    defaults = {
        ...
        "public_token"   : None,
        "role"           : "user",   # "sovereign" | "staff" | "user"
    }
...
def has_full_access():
    return S.is_subscriber or S.is_trial
...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    def make_display(df, amount_col, label):
        ...
        out["Type"]           = df["_type"]
        out["Risk"]           = df["_risk"]
        return out.sort_values("Delta (₹)", key=abs, ascending=False)
...
<table width="600" cellpadding="0" cellspacing="0"
  ...
  <tr>
    <td style="padding:20px 32px;border-top:1px solid #1A3A6E;
               background:#071233;">
      <p style="color:#C2EDFB;font-size:12px;margin:0;line-height:1.6;
                text-align:center;">
        Audit Desk · ITC Forensic Engine for Practising CAs<br>
        itcrecon.in · support: hello@itcrecon.in<br>
...
def send_welcome_email_free_user(to_email: str):
    ...
    body = f"""
        ...
        <div style="background:#0D2157;border-left:3px solid #00CFFF;
          ...
          <p style="color:#C2EDFB;font-size:14px;line-height:2;margin:0;">
        ...
        <div style="background:#0D2157;border:1px solid #1A3A6E;
          ...
          <p style="color:#C2EDFB;font-size:13px;line-height:1.8;margin:0;">
...
def send_welcome_email_subscriber(to_email: str, plan: str,
    ...
    body = f"""
        ...
        <!-- What to expect -->
        <p style="color:#C2EDFB;font-size:14px;line-height:1.8;margin:0 0 16px 0;">
...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    TBL_HDR      = colors.HexColor("#C2EDFB")    # Pale Sky — table header BG
    ...
    # Re-alias S_h1 for any remaining references
    S_h1 = S_brand
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        story.append(Paragraph(
            ...
            ParagraphStyle("SupNote", fontName=FONT,
                           textColor=colors.HexColor("#C2EDFB"),
    ...
    cert_text = (
        ...
        "<link href='mailto:hello@itcrecon.in'><font color='#1a56db'><u>hello@itcrecon.in</u></font></link>  |  "
...
def page_login():
    ...
    if _handle_password_reset_from_url():
        return  # Show reset form only, skip normal login page

    # Check for public report token in URL params
    ...
    # ── live numbers for social proof ──────────────────────────
    try:
        total_itc = sum(
            r.get("wealth_found", 0)
            for r in (db("recon_results").select("wealth_found")
                      .execute().data or [])
        )
    ...
    eb_open    = sub_count < EARLY_BIRD_LIMIT
    slots_left = max(0, EARLY_BIRD_LIMIT - sub_count)
    ...
    /* Left panel — dark authority */
    .login-left {
        background: linear-gradient(160deg, #071233 0%, #111823 60%, #071233 100%);
        border-right: 1px solid #1A3A6E;
        padding: 60px 48px;
        display: flex;
    ...
    .eb-pill {
        ...
        align-items: center;
        gap: 8px;
        background: linear-gradient(90deg, #071233, #2e2200);
        border: 1px solid #00CFFF;
        border-radius: 100px;
        padding: 6px 14px;
        font-size: 0.82rem;
    ...
    # ════════════════════════════════════════════
    # LEFT — Brand + Social Proof + Features
    # ════════════════════════════════════════════
    with left_col:
        ...
        try:
            ...
            pass  # Logo missing — text brand renders below

        # Brand
        st.markdown("""
        <p class='brand-mark'>Audit Desk</p>
        <p class='brand-name'>Find the money<br>your clients left behind.</p>
        <p class='brand-tagline'>
        Cross-reference GSTR-1, GSTR-2B, and GSTR-3B in under 30 seconds.<br>
        Every rupee of unclaimed ITC — found, documented, recovered.
        </p>
        """, unsafe_allow_html=True)

        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)

        # Social proof — only show real numbers once meaningful.
        # Empty zeros on a public login page destroy credibility.
        ITC_SHOW_THRESHOLD = 100000   # show after ₹1L total found
        SUB_SHOW_THRESHOLD = 10       # show after 10 active subscribers

        show_itc = total_itc >= ITC_SHOW_THRESHOLD
        show_sub = sub_count >= SUB_SHOW_THRESHOLD

        if show_itc or show_sub:
            itc_display = (
                f"\u20b9{total_itc/10000000:.2f} Cr" if total_itc >= 10000000 else
                f"\u20b9{total_itc/100000:.1f} L"    if total_itc >= 100000   else
                f"\u20b9{total_itc:,.0f}"
            )
            parts = []
            if show_itc:
                parts.append(f"<div><p class=\'proof-number\'>{itc_display}</p>"
                             "<p class=\'proof-label\'>ITC Recovered Platform-Wide</p></div>")
            if show_itc and show_sub:
                parts.append("<div style=\'width:1px; background:#1A3A6E;\'></div>")
            if show_sub:
                parts.append(f"<div><p class=\'proof-number\'>{sub_count:,}</p>"
                             "<p class=\'proof-label\'>CAs Using Audit Desk</p></div>")
            st.markdown(f"""
        <div style='display:flex; gap:32px; margin-bottom:24px;
                    padding:16px 20px; background:#071233;
                    border:1px solid #1A3A6E; border-radius:10px;'>
            {"".join(parts)}
        </div>
        """, unsafe_allow_html=True)
        else:
            # No real numbers yet — show value props instead
            st.markdown("""
        <div style='margin-bottom:24px; padding:16px 20px; background:#071233;
                    border:1px solid #1A3A6E; border-radius:10px;'>
            <div style='display:flex; flex-direction:column; gap:10px;'>
                <div style='display:flex; align-items:center; gap:10px;'>
                    <span style='color:#00CFFF;'>✓</span>
                    <span style='color:#FFFFFF; font-size:0.88rem;'>
                    Finds unclaimed ITC across 3 GST returns in under 30 seconds
                    </span>
                </div>
                <div style='display:flex; align-items:center; gap:10px;'>
                    <span style='color:#00CFFF;'>✓</span>
                    <span style='color:#FFFFFF; font-size:0.88rem;'>
                    Invoice-level mismatch report — exact rupee, exact vendor
                    </span>
                </div>
                <div style='display:flex; align-items:center; gap:10px;'>
                    <span style='color:#00CFFF;'>✓</span>
                    <span style='color:#FFFFFF; font-size:0.88rem;'>
                    Certified PDF + shareable client link — generated instantly
                    </span>
                </div>
                <div style='display:flex; align-items:center; gap:10px;'>
                    <span style='color:#00CFFF;'>✓</span>
                    <span style='color:#FFFFFF; font-size:0.88rem;'>
                    GST Calculator, MSME 45-Day, Section 50 — free with every account
                    </span>
                </div>
            </div>
        </div>
        ...
        # Feature list
        features = [
            ("📐", "Triangle Recon Engine",
             "GSTR-1 × 2B × 3B — invoice-level mismatch in seconds"),
            ("📄", "Audit Desk Certified PDF",
             "Professional report with your firm name — ready to send"),
            ("🔗", "Shareable Client Portal",
             "Send a link. Your client sees the recovery. You look like a hero."),
            ("🧮", "Free Daily Calculators",
             "GST, MSME 45-Day, Section 50 interest — open every day"),
            ("📊", "Monthly ITC Health Dashboard",
             "All clients. Green/yellow/red. One view. Every month."),
        ]
        for icon, title, sub in features:
            ...
            <div class='feature-row'>
                ...
                <div>
                    <span class='feature-text'><b>{title}</b></span><br>
                    <span class='feature-sub'>{sub}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <p style='color:#C2EDFB; font-size:0.75rem; border-top:1px solid #1A3A6E;
                  padding-top:16px;'>
        Used by practising CAs across India ·
        ₹999/month · No lock-in · Cancel anytime
        </p>
        """, unsafe_allow_html=True)
    ...
    with right_col:
        ...
        <div style='background:#071233; border:1px solid #1A3A6E;
                    border-radius:14px; padding:28px 28px 20px 28px;'>
        """, unsafe_allow_html=True)
        ...
        if eb_open:
            st.markdown("""
            <div class='eb-pill'>
                🔥 Early Bird — ₹999/month &amp; ₹8,999/year · Both prices locked forever · Limited time
            </div>
            """, unsafe_allow_html=True)
        ...
        with tab_login:
            ...
            if not st.session_state.show_reset:
                st.markdown("""
                <p class='footer-note'>
                <a href='#' style='color:#00CFFF; text-decoration:none;'>
        ...
        with tab_register:
            st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
            st.caption("Free account · 4 calculators + 14-day trial · no card needed")
            with st.form("reg_form"):
                r_email = st.text_input("Email address",
                             placeholder="ca@yourfirm.com",
                             key="reg_email",
                             autocomplete="email")
                r_pass  = st.text_input("Password",
                             type="password",
                             placeholder="Minimum 6 characters",
                             key="reg_pass",
                             autocomplete="new-password")
                r_pass2 = st.text_input("Confirm Password",
                             type="password",
            ...
            if reg_btn:
                if r_pass != r_pass2:
                    ...
                    user, err = get_or_create_free_user(
                        r_email.strip().lower(), r_pass)
                    if user:
                        _set_session_free(user)
                        st.rerun()
                    else:
            ...
            st.markdown("""
            <p class='footer-note'>
            No credit card needed. Calculators are free forever.<br>
            14-day full trial starts when you choose to activate it.
            </p>
            """, unsafe_allow_html=True)
        ...
        # Calculators require login — show teaser only
        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
        st.markdown("""
        <div style='border:1px solid #1A3A6E; border-radius:10px;
                    padding:14px 18px; text-align:center;
                    background:#071233;'>
            <span style='color:#00CFFF; font-size:0.8rem; font-weight:600;'>
            🧮 FREE WITH EVERY ACCOUNT
            </span><br>
            <span style='color:#C2EDFB; font-size:0.82rem;'>
            GST Calculator · MSME 45-Day · Section 50 · HSN Finder
            </span><br>
            <span style='color:#C2EDFB; font-size:0.75rem;'>
            Create a free account above to access — takes 30 seconds
            </span>
        </div>
        """, unsafe_allow_html=True)
...
def _handle_password_reset_from_url():
    ...
    with form_col:
        ...
        <p style='color:#C2EDFB; font-size:0.88rem; margin:0 0 20px 0;'>
    ...
    with st.sidebar:
        ...
        if role == "sovereign":
            ...
            <div style='background:#071233; border:1px solid #00CFFF;
                ...
                <span style='color:#C2EDFB; font-size:0.8rem;'>Full Access</span>
            </div>
            """, unsafe_allow_html=True)
        elif role == "staff":
            st.markdown("""
            <div style='background:#071233; border:1px solid #00CFFF;
                ...
                <span style='color:#C2EDFB; font-size:0.8rem;'>Developer Access</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style='background:#0D2157; border:1px solid #1A3A6E;
                        border-radius:8px; padding:10px 14px; margin-bottom:12px;'>
                <span style='color:#C2EDFB; font-size:0.85rem;'>Free Account</span>
            </div>
            """, unsafe_allow_html=True)
...
def page_dashboard():
    st.markdown("<h1 style='color:#FFFFFF;'>Dashboard</h1>",
                unsafe_allow_html=True)
    st.markdown(f"<p style='color:#C2EDFB;'>Welcome, "
    ...
    if not has_full_access():
        ...
        <b style='color:#00CFFF;'>🚀 Start Your 14-Day Free Trial</b><br>
        <span style='color:#FFFFFF;'>Full access to Triangle Recon, Client Roster,
        and Shareable Reports. No card needed.</span>
        </div>
        """, unsafe_allow_html=True)
        if st.button("Start Free Trial Now →", type="primary"):
            start_trial(S.email)
            S.free_user_row["trial_started"] = now_iso()
            S.is_trial = True
            st.rerun()
        return
    ...
    for i, (t, s) in enumerate([
        ...
        <div class='card' style='padding:14px 18px;'>
            ...
            <span style='color:#C2EDFB; font-size:0.85rem;'> — {s}</span>
        </div>
        """, unsafe_allow_html=True)
...
def page_recon():
    st.markdown("<h1 style='color:#FFFFFF;'>📐 Triangle Recon</h1>",
                unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#C2EDFB;'>GSTR-1 × GSTR-2B × GSTR-3B — "
        "Find every rupee of unclaimed ITC</p>",
        unsafe_allow_html=True)

    if not has_full_access():
        st.warning("Subscribe or start your free trial to use Triangle Recon.")
    ...
    if st.button("🚀 Run Triangle Recon", type="primary",
        ...
        with st.status("🔍 Interrogating GST data...", expanded=True) as status:
            ...
            try:
                ...
                # Sovereign and staff get full report immediately — no unlock step
                S.recon_unlocked = S.get("role") in ("sovereign", "staff")
                S.recon_saved_id = saved["id"] if saved else None
                S.recon_token    = saved["link_token"] if saved else ""
    ...
    if S.recon_result:
        ...
        if opportunity > 0:
            ...
            <div class='card card-amber' style='padding:20px; margin:12px 0;'>
                ...
                ₹0.00</p>
                <p style='color:#C2EDFB; font-size:0.85rem; margin:6px 0 0 0;'>
        ...
        <div style='display:flex; gap:16px; margin:16px 0;'>
            <div style='flex:1; background:#071233; border:1px solid #1A3A6E;
                ...
                <p style='color:#C2EDFB; font-size:0.75rem; letter-spacing:1px;
                ...
                <p style='color:#C2EDFB; font-size:0.78rem; margin:4px 0 0 0;'>
        ...
        if not S.recon_unlocked:
            ...
            <div style='background:#0D2157; border:2px solid #00CFFF;
                ...
                <p style='color:#00CFFF; font-weight:700; font-size:1.1rem; margin:0 0 12px 0;'>
                🔒 Full Report Locked
                </p>
                <div class='blur-content'>
Invoice No.    Supplier GSTIN      ITC in 2B    Tax in 1    Delta       Risk
INV/2025/001   27AAAAA0000A1Z5    ₹84,000      ₹72,000     ₹12,000     🔴 Critical
INV/2025/002   07AABCG1234A1Z2    ₹42,000      ₹38,000     ₹4,000      🟠 High
INV/2025/003   29AABCE1234A1Z3    ₹18,500      ₹15,000     ₹3,500      🟡 Medium
                ... {mismatch_count} rows hidden
                </div>
            </div>
            """.format(mismatch_count=summary['mismatch_count']),
            unsafe_allow_html=True)

            st.markdown("""
            <div class='card card-blue'>
            <b style='color:#FFFFFF;'>This is included in your ₹999/month subscription.</b><br>
            <span style='color:#C2EDFB;'>
            Unlock the full invoice-level breakdown, download the certified PDF,
            and get the shareable client link.
            </span>
            </div>
            """, unsafe_allow_html=True)

            if st.button("🔓 Unlock Full Report", type="primary",
                         use_container_width=True):
                if S.recon_saved_id:
                    mark_result_unlocked(S.recon_saved_id)
                S.recon_unlocked = True
                st.rerun()
            ...
            with tab4:
                ...
                if si_df is not None and not si_df.empty:
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.caption(
                        "Ranked by ITC at risk (highest first).  "
                        "Filing Rate = % of this supplier's own invoices that appeared in your GSTR-2B.  "
                        "Repeat Defaulter = filed <50% of their own invoices — consider ITC reversal.  "
            ...
            with col_link:
                token = S.get("recon_token","")
                if token:
                    share_url = f"{SITE_URL}?report={token}"
                    st.markdown(f"""
                    <div class='card card-green' style='padding:14px;'>
                    <b style='color:#00CFFF;'>📤 Client Share Link</b><br>
                    <span style='color:#C2EDFB; font-size:0.8rem; word-break:break-all;'>
                    {share_url}
                    </span><br>
                    <span style='color:#00CFFF; font-size:0.8rem;'>
                    Send this to your client via WhatsApp.
                    They see a clean read-only report — your name on it.
                    </span>
                    </div>
                    """, unsafe_allow_html=True)
...
def page_clients():
    st.markdown("<h1 style='color:#FFFFFF;'>👥 Client Roster</h1>",
                unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#C2EDFB;'>Register clients once. "
    ...
    for c in clients:
        with st.container():
            ...
            <div class='card' style='margin:6px 0; padding:14px 18px;'>
                ...
                <span style='color:#C2EDFB; margin-left:16px; font-size:0.85rem;'>
                {c.get('entity_type','')} ·
                GSTIN: {c.get('gstin','—')} ·
...
def page_calculators():
    st.markdown("<h1 style='color:#FFFFFF;'>🧮 Free Calculators</h1>",
                unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#C2EDFB;'>Free with every account — "
...
def page_subscribe():
    st.markdown("<h1 style='color:#FFFFFF;'>Subscribe to Audit Desk</h1>",
                unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class='price-card'>
            <p style='color:#C2EDFB; margin:0;'>MONTHLY</p>
    ...
    st.markdown("""
    <div style='text-align:center; color:#C2EDFB; font-size:0.85rem; margin-top:24px;'>
    ...
    <span style='color:#C2EDFB;'>14-day free trial · no card needed · cancel anytime</span>
    </div>
    """, unsafe_allow_html=True)
...
def page_public_report(token):
    ...
    <p style='color:#C2EDFB;'>Prepared by: <b style='color:#FFFFFF;'>
    {report['email']}</b></p>
    <p style='color:#C2EDFB;'>Client: <b style='color:#FFFFFF;'>
    {report['client_name']}</b></p>
    <p style='color:#C2EDFB;'>Period: {report.get('period','')}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.caption(
        f"Forensic Analysis by AuditDesk · itcrecon.in · "
        f"Report ID: {token} · "
...
def page_sovereign():
    ...
    <div style='background:linear-gradient(135deg,#071233 0%,#071233 100%);
        ...
        <p style='color:#C2EDFB; margin:0; font-size:0.9rem;'>
    ...
    for col, val, label, color in [
        ...
        (r4, f"₹{MONTHLY_PRICE:,}",   "Price Per Subscriber/Month","#C2EDFB"),
    ]:
        col.markdown(f"""
        <div style='background:#071233; border:1px solid #1A3A6E;
            ...
            <p style='color:#C2EDFB; font-size:0.75rem;
    ...
    for col, val, label, color in [
        ...
        (u5, f"{eb_slots:,}",       "Early Bird Slots Remaining","#C2EDFB"),
    ]:
        col.markdown(f"""
        <div style='background:#071233; border:1px solid #1A3A6E;
            ...
            <p style='color:#C2EDFB; font-size:0.72rem;
                      margin:6px 0 0 0;'>{label}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    for col, val, label, color in [
        ...
        (p4, f"{unlocked:,}",      "Full Reports Unlocked",               "#C2EDFB"),
    ]:
        col.markdown(f"""
        <div style='background:#071233; border:1px solid #1A3A6E;
                    border-radius:10px; padding:18px; text-align:center;
    ...
    <div style='background:linear-gradient(135deg,#071233 0%,#0D2157 100%);
        ...
        <span style='color:#C2EDFB; font-size:0.8rem; font-weight:400;'>
        as of {report_date}</span>
        </p>
        <div style='display:grid; grid-template-columns:1fr 1fr; gap:12px;'>
            <div style='background:#071233; border:1px solid #1A3A6E;
                        border-radius:8px; padding:16px;'>
                <p style='color:#C2EDFB; font-size:0.75rem; margin:0;'>MRR</p>
            ...
            <div style='background:#071233; border:1px solid #1A3A6E;
                        border-radius:8px; padding:16px;'>
                <p style='color:#C2EDFB; font-size:0.75rem; margin:0;'>ARR</p>
                <p style='color:#00CFFF; font-family:IBM Plex Mono;
                          font-size:1.4rem; font-weight:700; margin:4px 0;'>
                ₹{arr:,.0f}</p>
            </div>
            <div style='background:#071233; border:1px solid #1A3A6E;
                        border-radius:8px; padding:16px;'>
                <p style='color:#C2EDFB; font-size:0.75rem; margin:0;'>
...
def main():
    ...
    # Public report route — no login needed
    if "report" in params:
