...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    def make_display(df, amount_col, label):
        if df.empty:
            return pd.DataFrame()
        out = pd.DataFrame()
        out["Invoice No."]    = df["_inv_disp"].values
        out["Supplier GSTIN"] = df["_gstin_disp"].values
        out["Date"]           = df["_date_disp"].values
        out["ITC in 2B (₹)"]  = df["_itc"].round(2).values
        out["Tax in 1 (₹)"]   = df["_tax"].round(2).values
        out["Delta (₹)"]      = df["_delta"].round(2).values
        out["Type"]           = df["_type"].values
        out["Risk"]           = df["_risk"].values
        try:
    ...
    _parts = [x for x in [disp_missing_2b, disp_missing_1, disp_value_mm] if not x.empty]
    if _parts:
        all_mismatches = pd.concat(_parts, ignore_index=True)
        try:
