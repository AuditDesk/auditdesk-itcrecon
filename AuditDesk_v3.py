...
def get_or_create_free_user(email, password):
    ...
    try:
        ...
        if existing.data:
            stored = existing.data.get("password", "")
            if _verify_password(password, stored):
                ...
                if not stored.startswith("$2") and not stored.startswith("sha256$"):
                    db("free_users").update({"password": _hash_password(password)})\
                ...
                try:
                    ...
                    if not reset_row.data:
                        ...
                        if datetime.now() > expiry:
                            ...
                            db("free_users").update({"password": _hash_password(new_pass)})\
