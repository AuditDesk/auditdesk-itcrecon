...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    def build_supplier_intelligence(merged_df, gstr1_df, gstr2b_df):
        ...
        display["Compliance Score"] = si["filing_rate_pct"].apply(
            lambda x: f"{min(100, max(0, round(x))):.0f}%"
        )
...
def send_welcome_email_free_user(to_email: str):
    ...
    Goal: warm them up, explain Founding Member, make them want to subscribe.
    ...
    eb_block = """
        <div style="background:#071233;border:1px solid #00CFFF;
          ...
          <p style="color:#00CFFF;font-weight:700;font-size:15px;
                    margin:0 0 10px 0;">🏅 Founding Member offer — both plans locked forever</p>
          <p style="color:#FFFFFF;font-size:14px;line-height:1.8;margin:0;">
            Subscribe during Founding Member and your price is locked permanently —
    ...
    body = f"""
        ...
        <div style="background:#0D2157;border:1px solid #1A3A6E;
          ...
          <p style="color:#FFFFFF;font-size:13px;line-height:1.8;margin:0;">
            ...
            They get the same free account, same calculators, same Founding Member
    ...
    with col:
        ...
        with st.expander("🔑 Forgot your password?"):
            ...
            if st.button("Send Reset Link →", type="primary",
                         use_container_width=True, key="fp_send_btn"):
                _send_password_reset(fp_email.strip().lower())
...
def _send_password_reset(email):
    """
    Self-service password reset — uses Gmail SMTP (same system as welcome emails).
    1. Checks user exists in free_users table.
    2. Generates a secure token, stores it in password_resets with 1-hour expiry.
    3. Emails the user a reset link: https://itcrecon.in?reset=<token>
    4. When user clicks the link, _handle_password_reset_from_url() catches it
       and shows the new-password form automatically.
    """
    if not email:
    ...
    try:
        # ── 1. Check user exists (always show same message for security) ──
        user_check = db("free_users").select("email")\
            .eq("email", email.strip().lower()).maybe_single().execute()

        # Always show success — don't reveal whether email exists
        success_msg = ("✅ If that email is registered with us, a reset link "
        ...
        # ── 2. Generate token & store in Supabase ────────────────────────
        ...
        try:
            ...
            # password_resets table missing — tell user to contact support
            st.error(
                "Password reset is temporarily unavailable. "
                "Please email auditdesk.hq@gmail.com and we'll reset it manually."
            )
            return

        # ── 3. Send email via Gmail SMTP (same as welcome emails) ────────
        reset_url  = f"{SITE_URL}?reset={reset_token}"
        body_html  = f"""
        <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 16px 0;">
            You requested a password reset for your Audit Desk™ account.<br>
            Click the button below to set a new password. This link is valid for
            <b>1 hour</b> and can only be used once.
        </p>
        <div style="text-align:center;margin:28px 0;">
            <a href="{reset_url}"
               style="background:linear-gradient(135deg,#0095BF,#00CFFF);
                      color:#0A1B44;text-decoration:none;padding:14px 32px;
                      border-radius:8px;font-weight:700;font-size:15px;
                      display:inline-block;">
                Reset My Password →
            </a>
        </div>
        <p style="color:#FFFFFF;font-size:13px;line-height:1.6;opacity:.75;margin:0;">
            If you didn't request this, you can safely ignore this email —
            your password will not change.<br><br>
            If the button doesn't work, copy this link into your browser:<br>
            <span style="color:#00CFFF;word-break:break-all;">{reset_url}</span>
        </p>
        """
        sent = _send_email(
            to_email  = email.strip().lower(),
            subject   = "Reset your Audit Desk™ password",
        ...
        if sent:
            ...
            # Email sending failed — show a fallback support message
            st.warning(
                "We couldn't send the reset email right now. "
                "Please email auditdesk.hq@gmail.com with your registered email "
...
def page_recon():
    ...
    # ── GST Data Health Scanner ───────────────────────────────────────
    # Runs automatically when GSTR-1 is uploaded; shows a quick pre-check
    # before the main recon so the CA can fix data issues upfront.
    if f1:
        with st.expander("🩺 GST Data Health Scanner — auto-check your Purchase Register", expanded=True):
            try:
                import re as _re
                _gstin_pattern = _re.compile(
                    r"^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$"
                )
                def _read_uploaded(uf):
                    uf.seek(0)
                    return pd.read_csv(uf) if uf.name.endswith(".csv")                            else pd.read_excel(uf)

                _df_scan = _read_uploaded(f1)
                _total_rows = len(_df_scan)
                _issues = []

                # 1. Find GSTIN column
                _gstin_col = next(
                    (c for c in _df_scan.columns
                     if any(k in c.upper() for k in ["GSTIN","GSTNO","GST_NO","TAX_ID"])),
                    None
                )
                _inv_col = next(
                    (c for c in _df_scan.columns
                     if any(k in c.upper() for k in ["INVOICE","INV_NO","BILL_NO","INVNO"])),
                    None
                )
                ...
                _missing_gstin = 0
                if _gstin_col:
                    _gstins = _df_scan[_gstin_col].astype(str)
                    _missing_gstin = int(_gstins.str.strip().isin(["", "nan", "NaN", "None"]).sum())
                    _bad_gstin = int(
                        _gstins[~_gstins.str.strip().isin(["", "nan", "NaN", "None"])]
                        .apply(lambda x: not bool(_gstin_pattern.match(x.strip().upper())))
                        .sum()
                    )
                    if _missing_gstin:
                        _issues.append(("⚠️ Missing GSTIN",
                            f"{_missing_gstin} rows have no GSTIN — ITC cannot be claimed on these",
                            "amber"))
                    if _bad_gstin:
                        _issues.append(("❌ Invalid GSTIN format",
                            f"{_bad_gstin} GSTINs don't match the 15-character GST format",
                            "red"))

                # 2. Duplicate invoices
                _dup_count = 0
                if _inv_col:
                    _dup_count = int(_df_scan[_inv_col].duplicated().sum())
                    if _dup_count:
                        _issues.append(("🔁 Duplicate Invoices",
                            f"{_dup_count} duplicate invoice numbers — may inflate ITC claim",
                            "red"))

                # 3. Missing amount fields
                _amt_cols = [c for c in _df_scan.columns
                             if any(k in c.upper() for k in ["AMOUNT","AMT","VALUE","TAX","IGST","CGST","SGST"])]
                _missing_amt = 0
                for _ac in _amt_cols[:3]:
                    _missing_amt += int(_df_scan[_ac].isna().sum())
                if _missing_amt:
                    _issues.append(("⚠️ Missing Tax Amounts",
                        f"{_missing_amt} empty cells in tax/amount columns — reconciliation may be incomplete",
                        "amber"))

                # 4. Fully clean
                _clean = _total_rows - sum(
                    int(i[1].split()[0].replace(",","")) for i in _issues if i[1][0].isdigit()
                )

                # Display health report
                _h1, _h2, _h3 = st.columns(3)
                _h1.metric("Total Rows Scanned", f"{_total_rows:,}")
                _h2.metric("Issues Found", len(_issues),
                           delta=("⚠️ Fix before recon" if _issues else None),
                           delta_color="inverse")
                _h3.metric("Columns Detected", len(_df_scan.columns))

                if not _issues:
                    st.success("✅ Data looks clean! No formatting issues detected in your GSTR-1 file.")
                else:
                    for (_label, _detail, _sev) in _issues:
                        _col = "#e85d50" if _sev == "red" else "#f0a500"
                        st.markdown(f"""
                        <div style='background:#071233; border-left:4px solid {_col};
                                    border-radius:0 8px 8px 0; padding:10px 16px; margin:5px 0;'>
                            <span style='color:{_col}; font-weight:700; font-size:0.82rem;'>{_label}</span>
                            <span style='color:#FFFFFF; font-size:0.82rem;'> — {_detail}</span>
                        </div>
                        """, unsafe_allow_html=True)
                    st.caption("Fix these issues in your source file before running recon for best accuracy.")
            except Exception as _scan_err:
                st.caption(f"Health scan skipped: {_scan_err}")
    ...
    if S.recon_result:
        ...
        # ── ITC Leakage Finder ───────────────────────────────────────
        itc_2b      = summary.get("itc_available_2b", 0)
        itc_3b      = summary.get("itc_claimed_3b", 0)
        leakage     = max(0.0, round(itc_2b - itc_3b, 2))
        leakage_pct = round((leakage / itc_2b * 100), 1) if itc_2b > 0 else 0.0
        if leakage > 0:
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#0a2a0a,#0d3b0d);
                        border:2px solid #3fb950; border-radius:12px;
                        padding:18px 24px; margin:12px 0;
                        display:flex; align-items:center; gap:32px; flex-wrap:wrap;'>
                <div>
                    <p style='color:#3fb950; font-size:0.72rem; font-weight:700;
                               letter-spacing:2px; margin:0 0 4px 0;'>
                    💎 ITC LEAKAGE DETECTED</p>
                    <p style='color:#FFFFFF; font-size:1.7rem; font-weight:700;
                               font-family:monospace; margin:0;'>
                    ₹{leakage:,.0f}</p>
                    <p style='color:#FFFFFF; font-size:0.82rem; margin:6px 0 0 0;'>
                    Available in GSTR-2B but <b>not claimed</b> in GSTR-3B
                    ({leakage_pct}% of total ITC left on the table)</p>
                </div>
                <div style='flex:1; min-width:180px; background:rgba(63,185,80,0.08);
                            border-radius:8px; padding:12px 16px;'>
                    <p style='color:#FFFFFF; font-size:0.8rem; margin:0 0 6px 0;'>
                    <b style='color:#3fb950;'>ITC in GSTR-2B</b>  ₹{itc_2b:,.0f}</p>
                    <p style='color:#FFFFFF; font-size:0.8rem; margin:0 0 6px 0;'>
                    <b style='color:#00CFFF;'>ITC claimed in 3B</b>  ₹{itc_3b:,.0f}</p>
                    <p style='color:#FFFFFF; font-size:0.8rem; margin:0;
                               border-top:1px solid rgba(63,185,80,0.3); padding-top:6px;'>
                    <b style='color:#3fb950;'>Unclaimed ITC</b>  ₹{leakage:,.0f}</p>
                </div>
                <div style='font-size:0.8rem; color:#FFFFFF; opacity:.8;'>
                    ✅ Claim this ITC in your next 3B filing<br>
                    to recover ₹{leakage:,.0f} immediately.
                </div>
            </div>
            """, unsafe_allow_html=True)

        # ── GST Notice Risk Indicator ─────────────────────────────────
        _overclaim      = summary.get("itc_overclaim", 0)
        _missing2b      = summary.get("missing_in_2b_count", 0)
        _rep_defaulters = summary.get("repeat_defaulter_count", 0)
        _integrity      = summary.get("recon_integrity", 100)
        _opportunity    = summary.get("itc_opportunity", 0)
        ...
        if _overclaim > 0:
            _risk_score += 40
            _risk_flags.append(f"ITC overclaim of ₹{_overclaim:,.0f} detected in 3B")
        if _rep_defaulters > 0:
            _risk_score += 25
            _risk_flags.append(f"{_rep_defaulters} repeat-defaulter supplier(s) — ITC may be challenged")
        if _missing2b > 5:
            _risk_score += 15
            _risk_flags.append(f"{_missing2b} invoices missing from GSTR-2B — supplier filing gaps")
        if _integrity < 50:
            _risk_score += 20
            _risk_flags.append(f"Recon Integrity Score is {_integrity}% — high mismatch rate")
        elif _integrity < 75:
            _risk_score += 10

        if _risk_score >= 50:
            _risk_level = "HIGH"; _risk_color = "#e85d50"; _risk_bg = "#2a0a0a"
            _risk_icon  = "🔴"; _risk_border = "#e85d50"
        elif _risk_score >= 20:
            _risk_level = "MEDIUM"; _risk_color = "#f0a500"; _risk_bg = "#2a1f00"
            _risk_icon  = "🟡"; _risk_border = "#f0a500"
        else:
            _risk_level = "SAFE"; _risk_color = "#3fb950"; _risk_bg = "#0a2a0a"
            _risk_icon  = "🟢"; _risk_border = "#3fb950"

        _flags_html = "".join(
            f"<p style='color:#FFFFFF;font-size:0.8rem;margin:4px 0;'>"
            f"• {f}</p>" for f in _risk_flags
        ) if _risk_flags else (
            "<p style='color:#FFFFFF;font-size:0.8rem;margin:4px 0;opacity:.75;'>"
            "No critical risk factors detected.</p>"
        )

        st.markdown(f"""
        <div style='background:{_risk_bg}; border:2px solid {_risk_border};
                    border-radius:12px; padding:16px 24px; margin:12px 0;
                    display:flex; align-items:flex-start; gap:24px; flex-wrap:wrap;'>
            <div style='min-width:140px; text-align:center;'>
                <p style='color:{_risk_color}; font-size:0.72rem; font-weight:700;
                           letter-spacing:2px; margin:0 0 4px 0;'>GST NOTICE RISK</p>
                <p style='color:{_risk_color}; font-size:2rem; font-weight:900;
                           font-family:monospace; margin:0;'>{_risk_icon} {_risk_level}</p>
                <p style='color:#FFFFFF; font-size:0.72rem; margin:4px 0 0 0;
                           opacity:.7;'>Risk Score: {_risk_score}/100</p>
            </div>
            <div style='flex:1; min-width:200px; border-left:1px solid {_risk_border};
                        padding-left:20px;'>
                <p style='color:#FFFFFF; font-size:0.78rem; font-weight:700;
                           margin:0 0 8px 0;'>Risk Factors:</p>
                {_flags_html}
            </div>
        </div>
        """, unsafe_allow_html=True)
                ...
                if si_df is not None and not si_df.empty:
                    ...
                    def _color_compliance(val):
                        try:
                            ...
                            if pct >= 80: return "color: #3fb950; font-weight: bold"
                            if pct >= 50: return "color: #f0a500"
                            return "color: #e85d50; font-weight: bold"
                        except:
                            return ""

                    styled = (si_df.style
                        .map(_color_risk_cell,    subset=["Risk"])
                        .map(_color_defaulter,    subset=["Repeat Defaulter"])
                        .map(_color_compliance,   subset=["Compliance Score"])
                    )
