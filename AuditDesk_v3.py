...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        # Concentration note above the table — 3 lines, each stays inside box
        conc_note = (
            f"<b>{sup_count} suppliers</b> have invoices missing from GSTR-2B. "
            f"Top 5 account for <b>{top5_pct}%</b> of total gap.<br/>"
            f"<b>{rep_count}</b> supplier(s) flagged as <b>Repeat Defaulters</b> "
            f"(filed under 50% of their own invoices).<br/>"
            f"Assess ITC eligibility under Section 16(2) and evaluate reversal where applicable."
        )
        story.append(Paragraph(conc_note,
            ParagraphStyle("ConNote", fontName=FONT,
                           textColor=colors.HexColor("#24292f"),
                           fontSize=9, leading=15, wordWrap="LTR",
                           spaceAfter=2)))
        story.append(Spacer(1, 8))
        ...
        # Footnote — 3 lines so nothing clips at page edge
        footnote_text = (
            "<b>Filing Rate:</b> % of supplier invoices reflected in your GSTR-2B.<br/>"
            "<b>Repeat Defaulter:</b> Supplier filed under 50% of their own invoices "
            "— assess ITC eligibility under Section 16(2) and consider reversal.<br/>"
            "<b>Risk Score:</b> Composite score — ITC amount at risk (60 weight) "
            "+ filing failure rate (40 weight). Higher = more urgent follow-up."
        )
        story.append(Paragraph(
            footnote_text,
            ParagraphStyle("SupNote", fontName=FONT,
                           textColor=colors.HexColor("#334155"),
                           fontSize=7, leading=13, wordWrap="LTR",
    ...
    if summary:
        ...
        # Helper: wrap cell text as Paragraph for word-wrap + <br/> support
        _IS = ParagraphStyle("IS", fontName=FONT, fontSize=8, leading=12,
                             textColor=colors.HexColor("#24292f"), wordWrap="LTR")
        _ISH = ParagraphStyle("ISH", fontName=FONT_BOLD, fontSize=8, leading=12,
                              textColor=colors.white, wordWrap="LTR")
        def _ip(text, hdr=False):
            return Paragraph(str(text), _ISH if hdr else _IS)

        intel_data = [
            [_ip("Metric",hdr=True), _ip("Value",hdr=True), _ip("Interpretation",hdr=True)],
            [_ip("Recon Integrity Score"),
             _ip(f"{integrity}%"),
             _ip("Strong (&gt;75%) &nbsp; Moderate (50–75%) &nbsp; Weak (&lt;50%)")],
            [_ip("Invoice Mismatch Rate"),
             _ip(f"{mm_pct}%"),
             _ip(f"{summary.get('mismatch_count',0):,} of "
                 f"{summary.get('invoices_total', summary.get('invoices_2b',0)):,} "
                 f"combined unique invoices across GSTR-1 &amp; GSTR-2B")],
            [_ip("Net GST Exposure"),
             _ip(_pdf_rupee(net_exp)),
             _ip("Overclaim Risk + Value Mismatch Risk combined")],
            [_ip("Overclaim as % of 2B"),
             _ip(f"{overclaim_p}%"),
             _ip("No excess ITC claim detected. ITC claimed is within available GSTR-2B limits."
                 if overclaim_p == 0.0
                 else f"Excess ITC of {overclaim_p}% claimed vs GSTR-2B — reversal required.")],
            [_ip("Supplier Risk Concentration"),
             _ip(f"{top5_pct}%"),
             _ip(f"Top 5 suppliers account for {top5_pct}% of total supplier gap")],
            [_ip("Repeat Defaulters"),
             _ip(str(rep_count)),
             _ip(f"{rep_count} supplier(s) filed under 50% of their own invoices.<br/>"
                 f"Assess ITC eligibility under Section 16(2).<br/>"
                 f"Evaluate reversal where applicable.")],
            [_ip("Highest Single Supplier Risk"),
             _ip(_pdf_rupee(top1_amt)),
             _ip(f"GSTIN: {summary.get('top1_gstin','')} — largest single supplier exposure")],
        ]
...
def page_login():
    ...
    with col:
        ...
        # Check if forgot password mode is toggled
        ...
        if st.session_state.show_forgot_pw:
            ...
            <p style='color:#FFFFFF; font-size:1.3rem; font-weight:700;
                      margin:0 0 8px 0;'>Reset Your Password</p>
            <p style='color:#AABBCC; font-size:0.88rem; margin:0 0 20px 0;'>
            Enter your registered email. We'll send a reset link instantly.</p>
            """, unsafe_allow_html=True)
            ...
            with st.form("forgot_pw_form"):
                ...
                send_btn = st.form_submit_button("Send Reset Link →",
                                                  ...
                                                  type="primary")
            if send_btn:
                _send_password_reset(reset_email.strip().lower())
...
def _send_password_reset(email):
    ...
    try:
        ...
        reset_url = f"https://itcrecon.in?reset={reset_token}"

        # Send via Gmail SMTP (same system used for welcome emails)
        html_body = _email_html_wrapper(
            headline="Reset Your Password",
            body_html=f"""
            <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 16px 0;">
                We received a request to reset the password for your
                Audit Desk&#8482; account (<b>{email}</b>).
            </p>
            <p style="color:#FFFFFF;font-size:15px;line-height:1.8;margin:0 0 24px 0;">
                Click the button below to set a new password.
                This link is valid for <b>1 hour</b> and can only be used once.
            </p>
            <p style="color:#8899BB;font-size:13px;line-height:1.7;margin:24px 0 0 0;">
        ...
        sent = _send_email(email,
                           "Reset your Audit Desk™ password",
                           html_body)
        if sent:
            st.success("✅ Reset link sent! Check your inbox. "
                       "Link expires in 1 hour.")
            st.caption("Didn't receive it? Check your spam folder or "
                       "contact auditdesk.hq@gmail.com")
        else:
            st.error("Could not send reset email. "
                     "Please contact auditdesk.hq@gmail.com")
...
def page_recon():
    ...
    if S.recon_result:
        ...
        # ── ITC Leakage Finder ────────────────────────────────────────
        available_2b  = summary.get("itc_available_2b", 0)
        claimed_3b    = summary.get("itc_claimed_3b", 0)
        unclaimed_itc = summary.get("itc_opportunity", 0)
        if unclaimed_itc > 0:
            leakage_pct = round((unclaimed_itc / available_2b * 100), 1) if available_2b > 0 else 0
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#0a2e1a,#0d3d22);
                        border:1px solid #1f6b3a; border-left:5px solid #3fb950;
                        border-radius:10px; padding:20px 24px; margin:12px 0;
                        display:flex; align-items:center; gap:32px;'>
                <div style='flex:0 0 auto;'>
                    <p style='color:#3fb950; font-size:0.72rem; letter-spacing:2px;
                               font-family:monospace; margin:0 0 4px 0;'>💎 ITC LEAKAGE DETECTED</p>
                    <p style='color:#FFFFFF; font-size:2rem; font-weight:800;
                               font-family:monospace; margin:0;'>₹{unclaimed_itc:,.0f}</p>
                    <p style='color:#AAFFCC; font-size:0.82rem; margin:6px 0 0 0;'>
                        Available in GSTR‑2B but <b>not claimed</b> in GSTR‑3B
                        ({leakage_pct}% of total ITC left on the table)
                    </p>
                </div>
                <div style='flex:1; border-left:1px solid #1f6b3a; padding-left:28px;'>
                    <p style='color:#AAFFCC; font-size:0.82rem; margin:0 0 6px 0;'>
                        <span style='color:#3fb950;'>ITC in 2B</span>
                        &nbsp;₹{available_2b:,.0f}
                    </p>
                    <p style='color:#AAFFCC; font-size:0.82rem; margin:0 0 6px 0;'>
                        <span style='color:#3fb950;'>ITC claimed in 3B</span>
                        &nbsp;₹{claimed_3b:,.0f}
                    </p>
                    <p style='color:#FFFFFF; font-size:0.88rem; font-weight:700; margin:0;'>
                        <span style='color:#3fb950;'>Unclaimed ITC</span>
                        &nbsp;₹{unclaimed_itc:,.0f}
                    </p>
                </div>
                <div style='flex:0 0 200px; background:rgba(63,185,80,0.12);
                            border:1px solid #1f6b3a; border-radius:8px;
                            padding:12px 16px; font-size:0.8rem; color:#AAFFCC; line-height:1.6;'>
                    ✅ Claim this ITC in your next 3B filing<br/>
                    to recover <b style='color:#FFFFFF;'>₹{unclaimed_itc:,.0f}</b> immediately.
                </div>
            </div>
            """, unsafe_allow_html=True)
        elif overclaim > 0:
            # Show overclaim warning instead when 3B > 2B
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#2e0a0a,#3d0d0d);
                        border:1px solid #6b1f1f; border-left:5px solid #e85d50;
                        border-radius:10px; padding:20px 24px; margin:12px 0;'>
                <p style='color:#e85d50; font-size:0.72rem; letter-spacing:2px;
                           font-family:monospace; margin:0 0 4px 0;'>⚠️ ITC OVERCLAIM DETECTED</p>
                <p style='color:#FFFFFF; font-size:1.6rem; font-weight:800;
                           font-family:monospace; margin:0;'>₹{overclaim:,.0f} excess claim</p>
                <p style='color:#FFAAAA; font-size:0.82rem; margin:6px 0 0 0;'>
                    ITC claimed in 3B exceeds what is available in GSTR‑2B.
                    This triggers Section 50 interest (18% p.a.) + potential scrutiny.
                    Reverse the excess before your next filing.
                </p>
            </div>
            """, unsafe_allow_html=True)
