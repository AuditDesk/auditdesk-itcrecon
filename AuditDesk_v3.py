...
def _save_session_token(email):
    """Save a persistent session token to Supabase so next visit auto-logs in."""
    try:
...
def _verify_session_token(email, token):
    """Check if the session token in URL matches Supabase."""
...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    def build_supplier_intelligence(merged_df, gstr1_df, gstr2b_df):
        ...
        gstr1_df["_gstin_clean"] = (gstr1_df["_gstin"].astype(str).str.strip().str.upper().replace("NAN","UNKNOWN").replace("","UNKNOWN"))
        filed_per_gstin = (gstr1_df.groupby("_gstin_clean")
                           .size().reset_index(name="invoices_in_1")
                           .rename(columns={"_gstin_clean": "_gstin"}))
...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    TBL_HDR      = colors.HexColor("#c9a84c")    # Gold — table header BG
    TBL_HDR_TEXT = colors.white                  # White — header text
    ...
    # Wrapping cell styles for intel table
    ...
    if not mismatches_df.empty:
        ...
        page_w    = 7.0   # usable page width in inches
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        total_w_sup = sum(cw_list); page_w_sup = 7.0
    ...
    if summary:
        ...
        def _ic(txt): return Paragraph(str(txt), PS_cell)
        def _icb(txt): return Paragraph(str(txt), PS_cell_b)
        intel_data = [
            [_icb("Metric"), _icb("Value"), _icb("Interpretation")],
            ...
            [_ic("Overclaim as % of 2B"),
             _ic(f"{overclaim_p}%"),
             _ic("No excess ITC claim detected. ITC claimed is within available GSTR-2B limits." if overclaim_p == 0.0 else f"Excess ITC of {overclaim_p}% claimed relative to GSTR-2B — reversal required.")],
            [_ic("Supplier Risk Concentration"),
             _ic(f"{top5_pct}%"),
             _ic(f"Top 5 suppliers account for {top5_pct}% of total supplier gap")],
            [_ic("Repeat Defaulters"),
             _ic(str(rep_count)),
             _ic(f"{rep_count} supplier(s) filed <50% of their own invoices. Check eligibility u/s 16(2) CGST Act — ITC reversal may be required.")],
        ...
        t_intel = Table(intel_data, colWidths=[1.55*inch, 1.0*inch, 4.45*inch])
...
def page_login():
    ...
    if url_email:
        ...
        if email == sov_email:
            ...
            S.page = "sovereign"; S.remembered_email = email
    ...
    with col:
        try:
            ...
            pass
        # Check for remembered email in session
        remembered = st.session_state.get("remembered_email", "")
        remember_hint = (f"<p style='color:#00CFFF; font-size:0.82rem; margin:0 0 16px 0;'>"
                         f"Last signed in as <b>{remembered}</b></p>") if remembered else ""
        st.markdown(f"""
...
def sidebar():
    with st.sidebar:
        ...
        with st.expander("💬 Feedback / Issues"):
            suggestion = st.text_area("Tell us anything — bugs, ideas, issues",
                                      placeholder="e.g. Column mapping didn't work for my file...",
                                      key="sidebar_suggestion", height=80)
            if st.button("Send Feedback", key="send_suggestion", use_container_width=True):
                if suggestion.strip():
                    try:
                        ...
                        st.success("✅ Thank you! We'll look into it.")
                    except:
                        st.error("Couldn't send. Email us at auditdesk.hq@gmail.com")
                else:
        ...
        if not S.recon_unlocked:
            ...
            with col_link:
                ...
                if token:
                    ...
                    Copy and send to your client. They see a clean read-only report — your name on it.
                    </span>
                    </div>
                    """, unsafe_allow_html=True)
...
def page_sovereign():
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # SECTION G — GRANT FREE SUBSCRIPTION (Goodwill / Refund alternative)
    # ════════════════════════════════════════════════════════════════════════════
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.75rem;
    ...
    Use this to resolve subscriber issues — extend their subscription instead of a cash refund.
    </p>
    """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with g3:
            grant_reason = st.text_input(
                ...
                placeholder="e.g. PDF issue, goodwill",
                key="grant_reason",
                label_visibility="collapsed"
            )
        ...
        if st.button("🎁 Grant Free Subscription", type="primary",
            ...
            if not grant_email.strip():
                ...
                try:
                    ...
                    # Check if active subscription exists — extend it
                    existing = db("subscriptions").select("*") \
                    ...
                    if existing.data and len(existing.data) > 0:
                        ...
                        try:
                            ...
                            exp_dt = datetime.fromisoformat(
                                current_exp.replace("Z","").split(".")[0])
                            # Extend from current expiry (not today)
                        ...
                        for _ in range(months):
                            if new_exp.month == 12:
                                new_exp = new_exp.replace(year=new_exp.year+1, month=1)
                            else:
                                new_exp = new_exp.replace(month=new_exp.month+1)

                        db("subscriptions").update({
                            "expires_at": new_exp.isoformat(),
                            "grant_note": f"{grant_reason} [{grant_plan}] by sovereign on {now.strftime('%d-%m-%Y')}",
                        }).eq("id", sub["id"]).execute()
                        st.success(
                            f"✅ Extended {grant_email}'s subscription by {months} month(s). "
                            f"New expiry: {new_exp.strftime('%d %B %Y')}"
                        )
                    else:
                        # No active sub — create a free grant subscription
                        new_exp = now
                        for _ in range(months):
                            if new_exp.month == 12:
                                new_exp = new_exp.replace(year=new_exp.year+1, month=1)
                        ...
                        st.success(
                            f"✅ Granted {months} month(s) free subscription to {grant_email}. "
                            f"Access valid until {new_exp.strftime('%d %B %Y')}"
                        )
                except Exception as e:
                    st.error(f"Could not process grant: {e}")
...
def main():
    ...
    # Auto-login via session token (remember me)
    if not S.logged_in and "session" in params and "email" in params:
        ...
        if _verify_session_token(_email, _token):
            # Valid session token — auto-login without password
            try:
