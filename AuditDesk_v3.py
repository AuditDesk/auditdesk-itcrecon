...
def get_report_by_token(token):
    try:
        ...
        if res.data:
            return res.data
        # Fallback: token exists but not marked unlocked — fix and show
...
def _save_session_token(email):
    """Save persistent session token so next visit auto-logs in."""
    try:
    ...
    if _sponsor:
        ...
        <div style='background:linear-gradient(135deg,#071233,#0D1833);
            ...
            <span style='color:rgba(255,255,255,0.2); font-size:0.8rem; flex:1;'>
                Reach every CA who uses Audit Desk™ — daily, during live client work</span>
...
def page_sovereign():
    ...
    # ════════════════════════════════════════════════════════
    # SECTION G — GRANT FREE SUBSCRIPTION
    # ════════════════════════════════════════════════════════
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <p style='color:#00CFFF; font-family:IBM Plex Mono; font-size:0.75rem;
                ...
                if existing.data and len(existing.data) > 0:
                    ...
                    try:
                        exp_dt = datetime.fromisoformat(
                            sub.get("expires_at","").replace("Z","").split(".")[0])
                    ...
                    db("subscriptions").update({
                        ...
                        "grant_note": f"{grant_reason} [{grant_plan}] {now.strftime('%d-%m-%Y')}",
                    }).eq("id", sub["id"]).execute()
                    st.success(f"✅ Extended {grant_email}'s subscription by {months} month(s). "
                    ...
                    st.success(f"✅ Granted {months} month(s) free to {grant_email}. "
                               f"Valid until {new_exp.strftime('%d %B %Y')}")
            except Exception as e:
