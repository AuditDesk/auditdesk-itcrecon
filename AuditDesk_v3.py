...
def page_recon():
    ...
    if S.recon_result:
        ...
        # ── 🌟 Testimonial Nudge — shown when ITC opportunity found ────
        if opportunity > 0:
            st.markdown("---")
            ca_display = S.get("name", S.email.split("@")[0].title()) if hasattr(S, "get") else S.email.split("@")[0].title()
            default_text = (
            ...
            with st.expander("\u2b50 Impressed? Share your experience \u2014 takes 10 seconds", expanded=False):
                st.caption(
                    "Your testimonial may appear on itcrecon.in to help other CAs "
                    "discover this tool. You choose whether to submit."
                )
