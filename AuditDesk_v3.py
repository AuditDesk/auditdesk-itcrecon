...
def sidebar():
    with st.sidebar:
        ...
        # Suggestion box
        with st.expander("💬 Feedback / Issues / Ideas"):
            st.markdown("<p style='color:#FFFFFF;font-size:0.82rem;margin:0 0 6px 0;'>"
                        "Tell us anything — bugs, ideas, issues. "
                        "The more detail the better!</p>",
                        unsafe_allow_html=True)
            suggestion = st.text_area(
                "Your message",
                placeholder="Describe your issue or idea in detail. "
                            "Mention column names, file types, or steps to reproduce a bug...",
                key="sidebar_suggestion",
                height=160,
                ...
                if suggestion.strip():
                    try:
                        ...
                        if attach is not None:
                            img_b64  = _b64.b64encode(attach.read()).decode()
                        ...
                        st.session_state["sidebar_suggestion"] = ""
                    except Exception as _fe:
                        st.error("Couldn't send. Email: auditdesk.hq@gmail.com")
                else:
...
def page_recon():
    ...
    if S.recon_result:
        ...
        if not S.recon_unlocked:
            ...
            # AI narrative not configured — message suppressed
            # ── End AI Narrative ─────────────────────────────────────────
            ...
            with col_link:
                ...
                if token:
                    ...
                    wa_text   = f"ITC Recon Report for {S.get('recon_client','your firm')} — view here: {share_url}"
                    ...
                    <span style='color:#FFFFFF; font-size:0.78rem; word-break:break-all;'>
                    {share_url}
                    </span><br><br>
                    <a href='{wa_url}' target='_blank'
                       style='background:#25D366; color:#fff; padding:8px 16px;
                              border-radius:6px; text-decoration:none; font-weight:700;
                              font-size:0.82rem; display:inline-block;'>
                       📱 Send via WhatsApp
                    </a>
                    &nbsp;
                    <span style='color:#FFFFFF; font-size:0.78rem;'>
                    Client sees a clean read-only report — your name on it.
                    </span>
                    </div>
                    """, unsafe_allow_html=True)
