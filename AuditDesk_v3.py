...
/* Download button */
[data-testid="stDownloadButton"]>button { background:linear-gradient(135deg,#0095BF,#00CFFF)!important;color:#0A1B44!important;border:none!important;border-radius:6px!important;font-weight:700!important;box-shadow:0 2px 12px rgba(0,207,255,0.35)!important; }
[data-testid="stDownloadButton"]>button:hover { box-shadow:0 6px 24px rgba(0,207,255,0.55)!important;transform:translateY(-1px)!important; }
/* Expander white text */
...
def send_welcome_email_subscriber(to_email: str, plan: str,
    ...
    subject = (
        f"Your Audit Desk {'Founding Member ' if early_bird else ''}subscription is confirmed"
    )
    html = _email_html_wrapper(
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        story.append(Paragraph(conc_note,
            ParagraphStyle("ConNote", fontName=FONT,
                           textColor=colors.HexColor("#24292f"),
                           fontSize=9, leading=14, wordWrap="LTR",
                           splitLongWords=True)))
        story.append(Spacer(1, 8))
        ...
        S_cell_s = ParagraphStyle("CellS", fontName=FONT, fontSize=7, leading=10, wordWrap="LTR", splitLongWords=True)
        S_hdr_s  = ParagraphStyle("HdrS",  fontName=FONT_BOLD, fontSize=7, leading=10, textColor=colors.white, wordWrap="LTR", splitLongWords=True)
        ...
        story.append(Paragraph(
            "<b>Key:</b><br/>"
            "Filing Rate = % of supplier invoices appearing in your GSTR-2B.<br/>"
            "Repeat Defaulter = filed under 50% of own invoices — consider ITC reversal.<br/>"
            "Risk Score = ITC amount at risk (60%) + filing failure rate (40%).",
