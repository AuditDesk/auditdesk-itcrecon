...
[data-testid="stFileUploadDropzone"] {
    background: #0D2157 !important;
    border: 1px dashed #00CFFF !important;
    border-radius: 8px !important;
}
...
.price-card:hover {
    ...
    border-color: #00CFFF;
    box-shadow: 0 8px 40px rgba(0,207,255,0.35);
}
.price-card:hover, .price-card.featured:hover {
    border-color: #00CFFF !important;
    box-shadow: 0 8px 40px rgba(0,207,255,0.35) !important;
}
.price-card.featured {
    border-color: #00CFFF;
...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    merged["_gstin_disp"] = (merged["_gstin_2b"].combine_first(
                             ...
                             .replace("NAN","UNKNOWN").replace("","UNKNOWN"))
    ...
    def build_supplier_intelligence(merged_df, gstr1_df, gstr2b_df):
        ...
        gstr1_df["_gstin_clean"] = (gstr1_df["_gstin"].astype(str)
                                    .str.strip().str.upper()
                                    .replace("NAN","UNKNOWN").replace("","UNKNOWN"))
...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    if not mismatches_df.empty:
        ...
        page_w    = 7.0   # usable page width in inches (A4 margins)
    ...
    if supplier_intel_df is not None and not supplier_intel_df.empty:
        ...
        if _stw > 7.1:
            _ss = 7.1 / _stw
            cw_list = [round(w * _ss, 3) for w in cw_list]
    ...
    if summary:
        ...
        t_intel = Table(intel_data, colWidths=[1.6*inch, 1.0*inch, 4.4*inch])
...
def page_recon():
    ...
    if st.button("🚀 Run Triangle Recon", type="primary",
        ...
        with st.status("🔍 Interrogating GST data...", expanded=True) as status:
            ...
            try:
                ...
                time.sleep(0.4)
                # Generate token first — PDF gets it even if DB save fails
    ...
    if S.recon_result:
        ...
        if not S.recon_unlocked:
            ...
            # AI key not configured — suppress message
            # ── End AI Narrative ─────────────────────────────────────────
            ...
            with col_link:
                ...
                if token:
                    ...
                    <span style='color:#FFFFFF; font-size:0.8rem; word-break:break-all;'>
                    {share_url}
                    </span><br>
                    <span style='color:#00CFFF; font-size:0.8rem;'>
