...
def run_triangle_recon(gstr1_df, gstr2b_df, gstr3b_df,
    ...
    # Different suppliers can share the same invoice number, so GSTIN+InvNo
    # is the correct key when both files carry the same Supplier GSTIN.
    # However, some G1 exports carry Customer GSTIN instead of Supplier GSTIN,
    # making GSTINs never match across files. We detect this by sampling shared
    # invoice numbers and measuring the GSTIN agreement rate.
    def _gstin_match_rate(sample=300):
        if gstin_col_2b is None or gstin_col_1 is None:
            return 0.0
        common = list(set(gstr2b_df["_inv"].tolist()) & set(gstr1_df["_inv"].tolist()))[:sample]
        if not common:
            return 0.0
        hits = 0
        g1_idx  = gstr1_df.set_index("_inv")["_gstin"]
        g2b_idx = gstr2b_df.set_index("_inv")["_gstin"]
        for inv in common:
            try:
                v1 = g1_idx.loc[inv]; v2 = g2b_idx.loc[inv]
                v1 = v1.iloc[0] if hasattr(v1, "iloc") else v1
                v2 = v2.iloc[0] if hasattr(v2, "iloc") else v2
                if v1 == v2 and v1 not in ("UNKNOWN", "NAN", "NONE", ""):
                    hits += 1
            except Exception:
                pass
    ...
    gstr2b_df["_ckey"] = (gstr2b_df["_gstin"] + "||" + gstr2b_df["_inv"] + "||" + gstr2b_df["_date"]) if _use_gstin else (gstr2b_df["_inv"] + "||" + gstr2b_df["_date"])
    gstr1_df["_ckey"]  = (gstr1_df["_gstin"]  + "||" + gstr1_df["_inv"]  + "||" + gstr1_df["_date"])  if _use_gstin else (gstr1_df["_inv"]  + "||" + gstr1_df["_date"])
