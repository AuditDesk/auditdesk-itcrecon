...
def generate_pdf(client_name, ca_name, summary, mismatches_df,
    ...
    if summary:
        ...
        intel_data = [
            ...
            ["Repeat Defaulters",
             str(rep_count),
             f"{rep_count} supplier(s) filed <50% of their invoices — check eligibility u/s 16(2) and consider ITC reversal."],
    ...
    with st.sidebar:
        ...
        with st.expander("💬 Feedback / Issues / Ideas"):
            suggestion = st.text_area(
                "Tell us anything — bugs, ideas, issues, screenshots welcome",
                placeholder="Describe your issue or idea in as much detail as you like. "
                            "Include column names, file types, or steps to reproduce if reporting a bug...",
                key="sidebar_suggestion", height=160)
                ...
                if suggestion.strip():
                    try:
                        ...
                        try:
                            ...
                            # Fallback: send without image if columns don't exist yet
                            db("suggestions").insert({
                                "email"   : S.email or "anonymous",
                                "message" : suggestion.strip(),
                            }).execute()
                        ...
                        st.error(f"Couldn't send via app. Email us: auditdesk.hq@gmail.com")
                else:
